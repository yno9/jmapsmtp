...
func doSend(target string) {
	...
	// smtpSend's STARTTLS block, which this file claimed to have copied and
	// had not. Its absence is why no comparison could see that the Rust
	// client never upgraded: neither side did, so both agreed.
	//
	// SMTP_INTEROP_CA adds one root for a test server's self-signed
	// certificate. Verification itself stays on, exactly as in smtpSend —
	// turning it off here would compare a protocol nobody ships.
	if ok, _ := c.Extension("STARTTLS"); ok {
		cfg2 := &tls.Config{ServerName: strings.SplitN(target, ":", 2)[0]}
		if ca := os.Getenv("SMTP_INTEROP_CA"); ca != "" {
			pem, err := os.ReadFile(ca)
			if err != nil {
				emit(sendResponse{Err: fmt.Sprintf("read SMTP_INTEROP_CA: %v", err)})
				return
			}
			pool := x509.NewCertPool()
			if !pool.AppendCertsFromPEM(pem) {
				emit(sendResponse{Err: "SMTP_INTEROP_CA held no certificate"})
				return
			}
			cfg2.RootCAs = pool
		}
		if err := c.StartTLS(cfg2); err != nil {
			// What smtpSend does: log and carry on. The next command fails on
			// the dead socket and the transcript records how far it got.
			fmt.Fprintf(os.Stderr, "[smtp] STARTTLS failed for %s: %v (continuing plaintext)\n", target, err)
		}
	}
