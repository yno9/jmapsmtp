# 作業記録 — go-jmapsmtp → Rust 移植

新しいエントリを**上に**追記する（最新が先頭）。
計画は [PLAN.md](PLAN.md)、凍結された仕様は `SPEC.md`（M1 で作成）。

書き方の方針:
- **判断とその理由**を残す。コードを読めばわかることは書かない
- 詰まった点・回り道は消さずに残す。同じ罠を二度踏まないため
- 各エントリに、実際に走らせて確認したコマンドと結果を添える

---

## 2026-08-06 — biset の DID バインドを実機で通した / 3 本目の 501 (`d834709`〜)

### `xtask bind-probe` — クライアント・リレー・anchor を一度に

biset の `src/did/binding.ts` が定義する文をそのまま署名する:

```
bind:<did>:<username>@<relayHost>:<unixSeconds>
```

`did:dht` を使う。**識別子そのものが ed25519 公開鍵**（z-base-32）なので
anchor はネットワーク無しで検証できる。
`did:webvh` の SCID は文書ログのハッシュなので、それで試すと
**バインド経路ではなく anchor のリゾルバ**を試すことになる。

z-base-32 は移植側の `jmapserver::diddht` を使っている。
biset の実装と 1 文字でも違えば anchor は別の公開鍵を読んで署名を弾くので、
**通ること自体がこのエンコーダの第三者検証**になっている。

**リレーは署名を検証しない。** 判定するのは anchor。
だからリレー側のテストでは「本物のクライアントがバインドできるか」は
何も言えないし、interop のスタブ anchor は言われた通りに答えるだけ。

### 実機の結果（`relay.biset.md` → 本物の anchor）

```
正常                    → 204
タイムスタンプ +2 時間  → 401  anchor ログ: binding timestamp out of window
署名だけ差し替え        → 401  anchor ログ: binding signature invalid
同じアカウントに別 DID  → 409  did mismatch for this identity
```

**Ok / Invalid / Conflict の 3 verdict すべてを本物の anchor で観測できた。**
interop のスタブが script している答えと一致する。

> 204 だけ見て終われば、**anchor が何でも受け入れる**場合と区別が付かない。
> 否定対照が 3 種とも別々の理由で発火したので、204 に意味がある。

---

### 片付けようとして 3 本目の 501 を見つけた

テスト DID が本物のレジストリに残るので `/admin/drain-anchor` を叩いたら **501**。

**自分の guard の陰に隠れていた。**
`dispatch` はハンドラに入る前にベアラを検査する。
未認証で叩けば 401 で止まるので、`server_interop` の
「admin ルートは 401」は**期待通りの記録として残っていた**。
**トークンを持ったうえで POST するものが無かった。**

部分的な drain は **502** で報告する（本文はレポート）。
`failed` に残った名前は**まだクレームを保持しているかもしれない**。
放置されたクレームは、正当に別のリレーがその名前を取ることを永久に妨げる。
ステータスだけ見る運用者に「安全」と言ってはいけない。

### クラスごと塞いだ: `no_route_answers_501`

3 本とも**リクエストの形**が理由で見逃されていた ——
コードの中身ではなく。だから個別にテストを足すのをやめて、
**ルート表そのものを歩く**テストにした。

全パターン × 全メソッド × 認証情報とベアラの両方を持たせて、
**501 が 1 つでもあれば落とす**。anchored と anchorless の 2 回。
oracle 不要なので常に走る。

何を返すかは見ない。405 も 400 も 404 も
「ハンドラが走って意見を持った」なので合格。**501 だけが無人**。

2 つの arm を順に外して両方赤になることを確認した。
さらに**未登録パスが 404 であること**も要求している ——
501 がどこからも出なくなる変更が入ると、
この掃除が**中身のない緑**になってしまうので。

### drain の報告を信じずに確かめた

`released` と返ってきたが anchor のディレクトリに `alice` が残っていた。
**中身は空**で、決定的な確認は「別の DID を bind できるか」——
drain 前は 409、drain 後は **204**。解放は本物だった。

---

### 24 時間動作を「観測できる」形にした

`jmapsmtp-health.timer` が 5 分ごとに 1 行の JSON を追記する。
`MainPID` を入れてあるのは、**クラッシュループしていても
どのサンプルでも active/running に見える**から。
メッセージ件数も入れてある —— 「落ちなかった」と
「落ちなかったが 1 通失った」は別の話なので。

---

## 2026-08-06 — `Email/query` の 0.83× を 2.59× にした (`d834709`)

M8b のベンチが出した**唯一の負け**を潰した。

### 原因は「忠実に移植したこと」だった

`Email/query` は `Store::all()` を呼び、返ってきた全メッセージから
**id だけ**取り出していた。

Go の `All()` も同じ形をしている。だが **Go の struct コピーは
string / slice の「ヘッダ」を複製するだけ**で、中身のバイトは共有される。
**Rust の `.cloned()` はディープ**で、ヘッダ文字列も本文もマップも
全部確保し直す。

**同じ形のコードが、2 つの言語でまったく違う値段だった。**
移植でここが問題になったのは初めて。挙動の話ではないので
difftest でも interop でも出ない。

`Store::matching_ids` でロックの内側で絞り、
**生き残った id だけ** clone する形にした。

```
email-query（1,000 通 / 300 回）
  before  go 1.809ms / rust 2.173ms   0.83×  ← 移植側が遅い
  after   go 1.804ms / rust 0.697ms   2.59×
```

常駐メモリも 16.3MB → 11.3MB。
**クエリのたびにメールボックス全体を確保しなくなった**ため。

### 順序は `all()` と完全一致でなければならない

クライアントがページングするので、
ページ間で並びが変わると**行が消えるか二重に出る**。

両方とも `msgs` を BTreeMap 順に走査し、
**安定ソート**で負のキーを使う ―― 同着は id 順で確定する。

### そのテストは 60 件必要だった

最初 5 件で書いたら、**unstable ソートに変異させても通った**。
同着が少なすぎて、順序が狂う余地が無かった。

60 件（1 値あたり 20 の同着）にしたら落ちるようになった。

> **変異がすり抜けたら、assertion より先に入力を疑う。**
> M6y の「1 ミリ秒以内の 3 回」、M8b の「HTML パートの無いメール」と同じ形。

### 併せて: ファイアウォールを unit に持たせた

2525 を守る iptables ルールが**再起動で消える**問題。
`iptables-persistent` を入れるのではなく `ExecStartPre` にした ――
**ルールはポートを開ける当のサービスに属する**（サービスを止めれば
守るべきものも無い）。`-C` で存在確認してから `-A` するので
再起動しても重複しない。ルールを全削除してから
restart して復活すること、2 回目で 3 本のままであることを確認した。

---

## 2026-08-06 — M9（`/account/did` と `/pkarr/`）完了 (`4ae61df` / `7ce37ee`)

**deploy して初めて見つかった。** v2 に立てた試験リレーに anchor を繋いだら、
`/account/did` が **501** を返した。Go は `anchor_on.go` の `registerDidUpdate`
で認証してから anchor に DID クレームを転送する。移植側は `dispatch` に
arm が無く `_ => 501` に落ちていた。`/pkarr/` も同じ。

### 3 層の比較が、それぞれ別の理由で見逃していた

- `mux_interop` は**ルート表**を比較する。両側に存在するので一致し、
  **ハンドラについては何も言わない**
- `server_interop` の未配線リストは**空にされていた**。
  `let unwired: [&str; 0] = []` は**ループが 0 回**なので、
  緑に見えて**何も検査していなかった**。
  空にしたとき "Everything the oracle serves is now wired" と
  コメントまで書いている
- difftest のシナリオはどちらのパスも叩かない

**継ぎ目に落ちていた。**

### 根本原因は「anchor 有効時の表面を一度も比較していなかった」こと

これまでの interop は**全部 anchorless の oracle** だった。
だから穴を塞ぐのに、テストを 1 本足すのではなく
**無かった層そのもの**を作った — `did_bind_interop` が
スタブ anchor を立てて**両実装をそこに向ける**。

スタブは**リクエスト中の DID から**応答を決める。
呼ばれた順序でスクリプトすると、
**どちらが先に走ったかで結果が変わる**比較になってしまう。

anchorless の組も走らせている。
**判定順序は anchorless でしか観測できない**（anchor があると
「anchor が無い」の分岐に到達しない）。
そしてそこだけが「Go のソースを読んで書いた」箇所だったので、
oracle に聞いて確定させた。

---

### 途中で見つけたバグ 1: anchor 呼び出しが runtime 依存で panic する

`reqwest::blocking` は async コンテキスト検出のために `Runtime` を drop する。
その drop は「blocking が許されない場所」で panic する。

**multi-thread では発火しない。** 本番は `#[tokio::main]`（multi-thread）なので、
**v2 の稼働バイナリは実際に生きた anchor と喋って正しく答えていた** —
コードを変える前に実機で確認した。

だが「panic するかが**呼び出し側の runtime の作り方**次第」は
残す価値のない性質なので、`anchor::off_runtime` で
runtime の付いていないスレッドに出した。

**テストの fixture は current_thread のままにした。**
厳しい方を捨てると、再発を検出できなくなる。

### 途中で見つけたバグ 2: `just difftest` が移植側を再ビルドしない

古い `target/debug/jmapsmtp` と新しい oracle を比較して、
**ソース上では既に直っている差分**を報告してきた。

危険なのは逆方向 —— **古いバイナリが通ってしまう**方 —— なので
`difftest: build` の依存を足した。

### スタブが Go のリクエストを読めていなかった

`/pkarr/` の PUT 比較で「oracle が空の body を転送している」ように見えた。
**転送しているバイトは同じで、フレーミングが違った** —
Go は `r.Body` を `http.NewRequest` に渡すので長さが分からず **chunked**、
移植側はバッファして **Content-Length**。
スタブが Content-Length しか読まないので、
**片側だけ空に見えて、スタブ自身の差分を報告していた**。

スタブに chunked を読ませた。回避しなかったのは、
**片側のリクエストを読めない fixture は、2 つの実装ではなく
「実装と誤解」を比べている**から。

応答本文には **UTF-8 として不正なバイト**を入れてある。
blob をテキストとして decode する実装があれば、
fixture がたまたま ASCII だったせいで通る、ということが起きない。

---

### 変異テスト

`/account/did`: conflict→400 / 認証をボディ解析の後へ / CORS メソッド変更 —— すべて赤。
順序入れ替えは lib が捕捉、anchorless 組を足した後は interop も捕捉
（足す前はすり抜けた。**それがこの組を作った理由**）。

`/pkarr/`: キー検査とメソッド検査の順序入れ替え
（不正キーはどう叩かれても 404。405 にすると**パスの存在を教えてしまう**）/
404→400 / Content-Type を引き継がない —— すべて赤。

### 変異テスト中に自分でやらかした

`git checkout` で変異を戻したら、**未コミットのハンドラごと消えた**。
以降は**変異前にコミットする**。

### 実機確認

```
PUT /account/did  {}                        → 400 did required
                  {"did":...}               → 400 did_sig required
                  {"did":...,"did_sig":...} → 401 did binding rejected
   （anchor のログ: binding timestamp out of window ＝ 本物が判定している）
GET /pkarr/probekey → 400 "invalid key"
   anchor に直接叩いた応答と **status/Content-Type/本文が完全一致**
```

**oracle が出すルートは全部埋まった。** 780 件 / 36 スイート、
difftest 49 steps 差分なし。

---

## 2026-08-05 — M8b（ドキュメントとベンチ）完了 = **M8 完了** (`1f719bc` / `e888d9d` / `0d371e9`)

ARC.md・MIGRATION.md・README.md と `just bench`。
テスト 757 件（35 スイート）green、difftest 3 モードとも green。

### ARC.md は Go の ARC.md の翻訳にしなかった

先頭を「**アイデンティティはアドレスではなく DID**」にした。
これを掴んでいないと、`provision.go` が DID を保存しないことも、
`did:dht` はアンカー無しで検証できて `did:webvh` はできないことも、
biset と署名文字列をバイト単位で共有していることも、
**全部ただの実装詳細に見える**。

### MIGRATION.md の 1 番目は `ADMIN_TOKEN`

§11.13 が「**既存デプロイの見え方が変わる唯一の差異**」だから。
トークン未設定のまま切り替えると、その瞬間から scrape が 401 になる。
2 番目は「アカウント数が減る」(§11.16)。
水増しされていたのが直るのだが、**ダッシュボードはそれを知らない**。

ドキュメントの記述は**全部 oracle を起動して確認した**。
`go_*`/`process_*` が実在すること（Go 側 44 系統、移植側 4 系統）、
blob ルートが本当に mount されていないこと
（mount されたルートは未認証で 401、`/jmap/upload/` は 404）、
config.json を argv[0] のディレクトリから読むこと。

blob については**セッション文書が到達不能な URL を広告している**ことも分かった
（`NewMux` が `uploadUrl`/`downloadUrl` を無条件に埋める）。
移植側も同じものを広告する。Go のセッションで動いているクライアントは
これに耐えているはずで、直すと**そちらが壊れる**。

---

### `just bench` — 「動いているバイナリに訊く」を速度にも適用した

移植は**バイト単位で正しくて桁違いに遅い**ことがありうる。
これまでのハーネスはそれを何も言わない。

4 つ、明示的にやらないことを決めた:

- **並行しない。** 1 接続で順番に。
  スループットは実行機の性質であって、移植の性質ではない
- **平均を出さない。** median と p95。
  GC の 1 回のポーズが平均を動かして、典型的なリクエストの話をしなくなる
- **debug ビルドを拒否する。** 最適化された Go と比べても意味がない
- **`check` に入れない。** 時間は揺れる。
  実行機の機嫌で受け入れテストが落ちてはいけない

### ステータスの見張りが初回で仕事をした

`metrics` が移植側で 401 を返して**中断した**。
§11.13 そのもの（トークン未設定でルートが閉じる）なので、
そのまま測っていたら**ページ生成と拒否を比べた数字**が出ていた。

---

### 1,000 通を仕込もうとして、Go が受理したメールを捨てているのを見つけた

PLAN の M8 完了条件が「1,000 通での `Email/query`」と書いてあるので
SMTP で 1,000 通配送したら、**oracle は 256 件しか持っていなかった**。

Go は受信メールを到着時に保存しない。
容量 256 のチャネルに積み、**JMAP リクエストが来たとき**に流し込む。
間に 257 通目が来ると `default` 節に落ちて**捨てる** —
**SMTP ではすでに 250 を返した後**で。
送信側 MTA にとって 250 は「再送するな」なので、**誰も気づかない**。

移植側は到着時に保存するので、溢れる場所が無い。**再現しない**（§11.17）。

テストは 2 つで 1 組にした。片方だけでは、もう片方が変わったときに黙って通る:

- 移植側が 300 通すべて保存すること
- **oracle が今も捨てること**。Go 側が直れば「宣言が陳腐化した」として落ちる

> ここは**差分ハーネスが構造的に見られない領域**だった。
> difftest は HTTP しか叩かない。
> ベンチのために「実データを入れる」必要が出て初めて SMTP を通った。

### その テストが 3 回に 1 回落ちて、もう 1 つ見つかった (§11.18)

`Connection refused`。
Go は `go startSMTP(...)` で goroutine に投げてから HTTP を serve するので、
**HTTP が答え始めた時点でポート 25 がまだ上がっていない**ことがある。

ヘルスチェックが HTTP だけを見ていると、
**JMAP は健全でメールは拒否しているリレー**が見える。
移植側は SMTP を先に bind して await する（ARC.md §5 が主張していたことに、
実測が付いた）。

### ベンチ自身のバグを 2 つ踏んだ。どちらも「意味の無い数字」を出す形

1. **fixture はメールを 1 通も仕込まない。**
   初版は空のメールボックスに対して `Email/query` を測って、
   「ストアのソートとフィルタの経路」と名乗っていた。
   実際には `relay-info` と同じものを測っていた
2. **バッチごとに Message-ID を 0 から振り直していた。**
   ストアは Message-ID がキーなので後のバッチが前を上書きし、
   メールボックスが 1 バッチ分から増えない。
   **これは §11.17 の取りこぼしと見分けが付かない形で出た**

配送後に「送った数だけ入っているか」を確認してから測る形にした。

### 出た数字。1 つだけ悪い

```
case               go p50   rust p50     go p95   rust p95      p50
relay-info         0.301ms    0.273ms    0.341ms    0.329ms    1.10×
session            0.357ms    0.329ms    0.388ms    0.375ms    1.09×
mailbox-get        0.426ms    0.366ms    0.458ms    0.413ms    1.16×
email-query        1.809ms    2.173ms    2.518ms    3.107ms    0.83×
metrics            5.703ms    3.981ms    6.232ms    4.234ms    1.43×
startup               58ms       39ms          —          —    1.49×
resident            19.9MB     16.3MB          —          —    1.23×
```

**`Email/query`（1,000 通）だけ移植側が 10〜20% 遅い**（0.83〜0.88×、2 回とも）。
起動・常駐・小さいルートは移植側が上。

**追いかけていない。** M8b はドキュメントとベンチの整備であって、
最適化ではない。**数字を出すことと直すことは別の作業**なので、
直したくなったときのために PLAN に残した。

`metrics` の 1.43× は速度ではなく**出力量**である
（Go は go ランタイムのコレクタを 40 系統多く出す）。ヘッダにそう書いた。

---

## 2026-08-02 — M8a（difftest が oracle 対 Rust で通った）(`11f7906`)

**M4 から先送りしていた受け入れ条件を満たした。**
`just difftest` が 46 ステップ全部で差異なし。

### 初回実行で本物のバグを 1 件見つけた

`POST /jmap/api/` の不正ボディに対して、
Go は `bad request`、移植側は `invalid request` を返していた。

**これを見つけられるものは他に無い** —
単体テストは自分で定義した文字列としか照合できない。

### 宣言済み差異の機構（SPEC §11.9）をやっと作った

ステップに**「何が」違うか（`Aspect`）と「なぜ」**を書く。
宣言された側面**だけ**が比較から外れ、それ以外は通常どおり比較される。

### その最初の実装が間違っていた（記録しておく価値がある）

初版は**「ステップ全体が同一か」**で判定していた。

`/metrics` は**どのみち body が違う**
（Go ランタイムのコレクタに移植側の対応物が無い）。
だから §11.13 の bearer 修正を**わざと失わせても**、
body が違うままなので「まだ差異がある」として**通ってしまった**。

側面を名指しする形に直したら、同じ変異で落ちるようになった。

> **宣言済み差異は、宣言が雑だと「何も検証しないテスト」に化ける。**

### 同一実装同士では宣言を無効化する

`--both-oracle` と `--self-test` は同じ実装を両側に置く。
そこで差異を要求すると、**唯一「異常でない理由」で自己検査が落ちる**。

### サーバログは「行の集合」として比較する

起動ログは並行処理から出る（Go は SMTP 行を goroutine から書く）ので、
**どちらの実装も順序を決定していない**。
契約は「同じ事象が報告されること」で、
片側にしか無い行は集合比較でも出る。

### 正規化フィルタを置く場所を 1 回間違えた

バイナリ名の接頭辞を `^` 固定のパターンで消そうとしたが、
**Go のタイムスタンプを剥がす前**に走っていたので
**移植側にしか当たらなかった**。後ろに移して、理由をその場に書いた。

---

## 2026-08-02 — M7c（Web Push 送信）完了 = **M7 完了** (`c0558f8`)

**SPEC §2 の起動シーケンスが全ステップ組み上がった。**
テスト 749 件（34 スイート）green。

### 通知は「何も運ばない」

payload は変化した capability の名前だけ。
event-source のフレームと**バイト一致**なので、クライアントは 1 経路で扱える。

送信者や件名を入れると、
**リレーが管理せず、ユーザが選んでもいない第三者（push サービス）に
メタデータを渡す**ことになる。

payload にアドレスを足す変異は 2 件のテストで落ちる。

### 依存の入れ方

RFC 8291 の暗号化と VAPID JWT は `web-push` から。ただし **client feature 無し** —
crate のクライアントは**第二の TLS スタック**を引き込むので、
送信は既にある reqwest でやる。

リクエストは crate の `build_request` を使わず**フィールドから組み立てた**。
あちらは `http` 0.2 を返し、こちらは 1 なので、
借りると**同じ crate の 2 バージョンを抱える**ことになる。

### 404 と 410 だけが購読を削除する

他は全部「このリレーから見て一時的」。
**429 で削除すると、レート制限に当たった瞬間に生きているクライアントを解約する。**

### JWT の `sub` は「デコードして」確認するようにした

ヘッダの接頭辞だけ見ていたら、**claim を消す変異が通った**。

`sub` が無い/壊れているのは
**Apple が 403 で拒否し Google は受け入れる**ケース —
片方のベンダでしか出ない失敗なので、推測ではなく検査した。

### spawn を守るのではなく、テストを async にした

`deliver` が push を spawn するのでランタイムが要る。
**spawn 側にガードを入れると、push 経路が一度も走らないままテストが通る。**
それはテストの目的の逆。

---

## 2026-08-02 — M7b（identity anchor クライアント）完了 (`87dce4f`)

テスト 737 件（34 スイート）green。

### リレーは証明を「転送する」のであって「検証しない」

DID の暗号が**1 箇所**に集まる。
リレー側で検証すると、**新しい DID メソッドが出るたびに全リレーを揃えて上げる**必要が出る。

例外は `did:dht` だけ（自己証明的なのでローカル検証）。

### 保つべき区別が 2 つ

**1. `Invalid` は「anchor が見て拒否した」、`Error` は「見てすらいない」。**

403 は**このリレーが門前払いされている**状態。
`Invalid` として報告すると、
**何も見られていないユーザに「あなたの DID 証明が失敗した」と言う**ことになる。
401 は 401、403 は 503。

**2. `Conflict` は「別の DID がその名前を持っている」。**

クライアントの次の一手は**別の名前を選ぶ**ことであって、署名し直すことではない。
`Invalid` と混ぜると、**問題でなかった鍵を導出し直させる**。

### claim は vouch より先

そのDIDが保持していない名前に対して vouch が通ると、
**他人のメールボックスにデバイスが紐づく**。

### anchor に到達できない場合、「未 anchor で進む」は絶対にない

未 bind の名前は**後から他人が claim できる**。
衝突は「元の持ち主がアドレスを失う」形で、
**それを起こしたリクエストのずっと後に**現れる。

アカウント削除は逆で best-effort — **ユーザは去ると言っている**。

### Host ヘッダは verbatim

クライアントが署名した対象であり、
**このリレーだけが一次情報として持っている**もの。
設定のホスト名で置き換えると、
**別のリレーで捕捉した署名をここで再生する**攻撃への防御が消える。

### reqwest の blocking クライアントは遅延生成に

**バックグラウンドランタイムを持っていて、async コンテキストで drop すると panic する。**
async テストで RelayState を作るたびに起きていた。
プロセスに 1 つ、drop しない形にした。

### 説明できない失敗を 1 件記録した

ある実行で **34 スイート中 13 で停止**した。その後 **25 回以上再現しない**。
原因も、どのテストだったかも掴めていない。

**滑らかにせず記録する** — 次に起きたときに「2 回目」だと分かるように。

---

## 2026-08-02 — M7a（受信 STARTTLS）完了 (`defd9e3`)

テスト 709 件（34 スイート）green。

### `handle` は Outcome を返し、ストリームを借りる

STARTTLS に 220 を返して**呼び出し側に返す**。
`smtp_in` は TLS 依存を持たず、素のバイトストリームでテストできるまま。

**アップグレード前に交渉したものは全部捨てる**（RFC 3207 §4.2）。
保護されていなかったので、**中間者が書き換えられた**可能性がある。

アップグレード後は STARTTLS を**再広告しない**。
これだけはハンドシェイクを完了しないと確認できないので、テストで完了させた。

### 自己署名は「弱い版の本物」ではない

**ポート 25 での代替は「検証済み TLS」ではなく「平文」。**
送信側 MX は任意の受信ドメインを認証できないので、
拒否ではなくフォールバックする。

**受動的な盗聴に対して暗号化する**ことが便益の全部であり、それは本物の便益。

### 更新は再起動なしで拾う

Caddy や certbot が更新するファイルを、**mtime が変わったら読み直す**。
再起動しないと新しい証明書を出せないリレーは、
**誰も気づかない間ずっと期限切れを出し続ける**。

書き込み途中を掴んだ場合は**キャッシュを出す** —
一時的な状態なのに TLS を落とすと、その間**全送信者が平文に落ちる**。

設定したペアがそもそも読めない場合は自己署名にフォールバックする。
**パスの打ち間違いで全送信者が黙って平文に落ちる**べきではない。

### rustls の crypto provider を明示的に入れた

feature の統合結果に任せない。
**どのビルドでも同じように失敗する**ほうが、
一部のビルドだけで失敗するよりよい。

---

## 2026-08-02 — M6z（送信サブミッション）完了 (`75b6e9a`)

**これで受信も送信も通る。** テスト 697 件（34 スイート）green。

### `outbound.rs` の順序は契約

build → Autocrypt → Chat-Version → PGP/MIME ラップ → **DKIM** → send。

各段が**前段の出力に署名/ラップする**。DKIM が最後なのは
**完成したメッセージに署名する**から — 後からヘッダを足すと署名が無効になる。

入れ替えても**エラーにはならない**。
**相手側で検証に失敗するメッセージが出来るだけ**。
だから完成バイト列に対して順序を assert した。

### 保存コピーと送信コピーは別のオブジェクト

受信者は平文を受け取り、ここに残るのは（鍵があれば）封印される。
**封印した方を送ると、鍵を持たない相手に暗号文が届く。**

暗号化に失敗した場合は**平文のまま保存して送信は続ける**。
メッセージは既に出ていくので、
ここで拒否すると**ユーザが消せるコピーを守るために本体を失う**。

### 変異テストがテスト側の欠陥を 3 つ暴いた

1. 「封印は送信内容を変えない」の初版は**渡したオブジェクト**を見ていた。
   送信経路が受け取ったものについて何も言っていない。
   **封印コピーを送る変異が通っていた。**
   → **ワイヤのバイト列**を読む形に変更
2. dump のパスは Go 実装が固定していて共有ファイルになる。
   ロックを**片方のテストモジュールに置くと、もう片方と排他できない**。
   `outbound` 側に置いて両方から使う。
   さらに await をまたぐので**非同期 Mutex**にした
3. モジュールが **26 秒**かかっていた。テストごとに RSA-2048 を生成していたから。
   共有 fixture を seed して **0.02 秒**。
   **テスト対象はパイプラインであって鍵生成器ではない**

### 「悪い変異」も 1 つあった

`outbound` のクローン位置を動かす変異を書いたが、
**封印はその後に起きる**ので意味的に何も変わらない。
テストが弱いのではなく**変異が的を外していた**。

送信引数そのものを封印コピーに差し替える変異に直したら、
3 回とも安定して落ちるようになった。

---

## 2026-08-02 — M6y（受信配送・SMTP リスナ・非活動アカウント掃除）完了 (`ba44b7e`)

SPEC §2 のステップ 13・14。テスト 680 件（34 スイート）green。

### SMTP は HTTP より先に bind する

**JMAP は受けるがメールは受けないリレー**は、
まだ起動していないリレーより悪い —
**監視から見ると健全に見える**から。

### 受信配送で守ったこと

- **知らない宛先も 250 で受けて捨てる。**
  RCPT で拒否すると、ポート 25 に到達できる誰にでも
  **どのアドレスが存在するか**を教えるオラクルになる
- 保存 id は **RFC Message-ID 由来**。
  リトライや 2 番目の MX が**重複ではなく上書き**になる
- **容量上限は入る側でも見る。** どちらの向きでもディスクは埋まる

### 掃除の条件は「残す理由」を個別に並べた

1 つの合成述語にしない。理由が互いに無関係で、
**将来の変更が「意図した理由だけ」を外す**必要があるから。

一番重いのは**ピア確認**:
同じアドレスを複数のリレーが持つので、
**静かな方で掃除すると、生きているアカウントのメールが半分消える**。

存在しないピアディレクトリは「活動なし」と読む —
一時的にマウントされていないピアを列挙しただけで
**掃除が黙って止まる**のを防ぐ。

### 変異 1 件が暴いたのは「コードの欠陥」ではなく「時間依存のテスト」

「同じメッセージの再配送は上書きされる」を**件数だけ**で見ていた。
Message-ID を使わない実装でも、
**1 ミリ秒以内の 3 回**は 1 件に潰れる。

**同じ変異が、実行ごとに通ったり落ちたりした。**

導出された **id そのもの**を assert する形に変え、
両方向で 3 回ずつ確認した。

（「緑だから正しい」だけでなく
**「赤が安定して赤か」**も見る必要がある、の実例）

### 残りを entry point に正直に書いた

`NOT_YET_WIRED`:
送信サブミッション、受信 STARTTLS、anchor クライアント。

---

## 2026-08-02 — M6x（Web Push と event-source）完了 (`c4d0e72`)

最後の 4 ルート。**oracle が提供する全ルートが配線された。**
テスト 661 件（34 スイート）green。

`server_interop` の未配線リストは**削除せず空にした** —
次に未配線ルートが来る場所であり、
そのループが「501 で黙って答えていないこと」の証明でもあるから。

### 送信はまだ入れていない（明記した）

`push.rs` は**購読レジストリと VAPID の身元**まで。
配信には RFC 8291 の暗号化と VAPID JWT が要り、それは別マイルストーン。
**モジュール冒頭に書いた** — 欠落が見落としに見えないように。

### 型が持つ 3 つの性質

1. **subscriber の `mailto:` を剥がす。**
   ライブラリ側が自分で付けるので、呼び出し側が付けると
   `mailto:mailto:…` になる。
   **Apple は 403 で拒否し、Google は受け入れる** — 片方のベンダでしか出ない失敗
2. **同じ endpoint を二重登録しない。**
   ページ読み込みごとに購読するクライアントは、
   **コピーの数だけ通知される**ことになる
3. **購読は永続化する。**
   ブラウザは促されずに再購読しないので、
   再起動で失われた購読は**永久に失われる** —
   ユーザは黙って通知されなくなり、手がかりが何も残らない

### ストリームは最初のフレームを即座に送る

変化が何時間先か分からないので、
**「生きている」ことをクライアントに伝える**ため。

`Connection` ヘッダは**付けない**。
HTTP/2 では hop-by-hop ヘッダはプロトコル違反でストリームがリセットされ、
**200 を送った後で `ERR_HTTP2_PROTOCOL_ERROR` として現れる**。

### interop は 401 までしか届かないので、ストリームは単体テスト

最初のフレーム、hub 通知後の 2 枚目、
そして keep-alive ヘッダが無いこと。3 つとも変異で落ちる。

---

## 2026-08-02 — M6w（カスタムドメインと DNS クライアント）完了 (`e9586f8`)

テスト 646 件（34 スイート）green。

### DNS は 2 つとも「重い判断」をする

| 検索 | 決めること | 誤ると |
|---|---|---|
| MX | メッセージの行き先 | **別のホストに配送する** |
| TXT | ドメインの所有権 | **持っていないドメインを渡す** |

両方トレイト越しにした。
上位の判断が**インターネットに聞かずにテストできる**のはこれのため。

### 失敗した検索と空の検索は同じ答え（＝拒否）

リゾルバの障害を「レコードが無い」以外に解釈すると、
**障害中にドメインを配ってしまう**。

既定のリゾルバは**何も答えない**ので、
未設定のリレーは**あらゆる所有権証明を拒否する**。
これ自体にテストを付けた（fail closed）。

### `/domain/verify-token` に特権情報は無い

公開鍵レコード、リレーのホスト名、
そして「**聞いた側が手順を読んだ**」ことしか証明しないトークン。
**provision_secret は入らない。**

ハンドラに secret を足す変異が比較で落ちることを確認した —
散文で主張せず**検査した**。

### `/domain/add` は毎回 TXT を確認し、gated で登録する

`allow_provision` にはしない。
**一度登録したら以後ずっと誰でもアカウントを作れる**状態にしないため。

成功パスは oracle 経由で駆動できない（誰も所有していないドメインに
生の TXT レコードが要る）ので、
**注入したリゾルバで単体テスト**にした。
再起動後も secret が保たれること（既に配ってしまっているので）まで含む。

### トークンを 2 回聞いても DKIM 鍵は回らない

既にレコードを公開した所有者が**黙って DKIM に失敗し始める**。
`load_or_generate_key` を再生成に変える変異が比較で落ちる。

---

## 2026-08-02 — M6v（管理ダッシュボードと Prometheus メトリクス）完了 (`71de8c1`)

テスト 640 件（34 スイート）green。

### ダッシュボードは逐語コピー

リリース済みクライアントであり、**取得する JSON は移植側の出力**。
書き直さずバイト比較。トークン無しで提供する —
**ページが行う全リクエストがトークンを持つ**ので、
シェル自体はアカウントデータを含まない。

### メトリクスは `biset_*` のみ

Go 版は Go ランタイムとプロセスのコレクタも登録しているが、
**それらは Go プロセスの話**で、こちらに対応物が無い。
似た数字を出すのは**数字の捏造**になる。

移行後、運用者が見るのは
**「違う系列」ではなく「少ない系列」**になる。

### 比較で判明した 2 つ

1. **アカウントが 0 のドメインディレクトリにも系列が要る。**
   `list_provisioned` だけから数えると**系列ごと消える**。
   アラートから見ると「0 になった」と「リレーが落ちた」が区別できない。
   ディレクトリ一覧から 0 を先に置き、
   件数は `/admin/accounts` と同じ walk から足す形にした
   （§11.16 の修正はそのまま効く）
2. `biset_smtp_outbound_total` は**両ラベルとも 0 で存在する**。
   最初のイベントまで現れないカウンタは `rate()` が未定義になる

### 露出フォーマット自体をテストした

- **メトリクス名ごとに HELP と TYPE は 1 回**、その下に全系列
  （同じ名前で HELP が繰り返されるとパースエラーになるスクレイパがある）
- gauge と counter を取り違えない（**counter を gauge で描くと形が変わる**）
- ラベル値のエスケープ（**引用符 1 つで以降の全行が壊れる**）

---

## 2026-08-02 — M6u（アカウントのライフサイクル）完了 (`a340b87`)

`/account/provision`、`/account/delete`、`/account/storage/purge-messages` を配線。
テスト 632 件（34 スイート）green。

### provision は「本物の did:dht 署名」で端から端まで比較

共有 data ディレクトリ上で**両サーバがそれぞれ別の名前を作り、
その後お互いの名前を拒否する**形にした。
衝突規則を**実装内ではなく実装間**で確認できる。

### 変異 3 件が通り抜けて、どれも「テストが届いていない場所」だった

1. **vouch 検証を飛ばしても緑**だった。
   拒否ケースが全部**もっと手前で**弾かれていたから（DID 無し、未知ドメイン）。
   **最後まで整形式で、署名だけが「別の statement に対する本物の ed25519」**
   というケースが必要だった。追加したら落ちるようになった
2. **did:webvh のケースが vouch 経路に到達していなかった** —
   ドメインゲートで先に 403 になっていた。開放ドメインに移した
3. **2 つの枝はどんな設定でも到達できない**:
   `did_bound` が省略される枝（`validate` が空 DID を先に拒否する）と、
   anchor 経由の vouch 枝（`anchor_url` が要り、
   かつ本物の anchor に到達しないと比較に意味が無い）。
   **「カバーされているように見える」のをやめ、その場に明記**した

### 単体テストが 3 回目の陳腐化

配線するたびに「未配線ルートの例」がハードコードされた単体テストが落ちる。
**ルート名を書くのをやめた** — 501 応答の**形**だけを固定し、
権威あるリストは oracle と比較している `server_interop` に一本化した。

### ハーネスの競合、2 件目

`free_port` は **127.0.0.1** で空きを確認していたが、
oracle の SMTP リスナは **`:<port>`（全インターフェース）** に bind する。

つまり**他プロセスが全インターフェースで掴んでいるポートが「空き」に見えた**。
oracle は `bind: address already in use` で死ぬ。

プローブを `0.0.0.0` に変えた —
**確認する bind を、その後で実際に行う bind と同じ強さにする**。
**10 回連続 green** を確認。

（1 件目は M6t の「別の oracle に繋がる」問題。
どちらも「再実行して緑」にせず原因を特定した）

---

## 2026-07-30 — M6t（JMAP 本体）完了 (`afb0836`)

`/.well-known/jmap` と `/jmap/api/` を配線。
起動シーケンスのステップ 11（アカウント毎の Store とエイリアス表）が必要だった。

テスト 630 件（34 スイート）green。

### Store が開けないアカウントは fatal

飛ばして続行すると、
**1 アカウントのメールを黙って捨てながら健全に見えるリレー**になる。

### accountId の解決が JMAP 層の唯一の隔離

credential は HTTP の境界で**1 回だけ**確認される。
その後は `accountId` の解決だけが「他人のデータを返さない」根拠。
知らないアカウントは拒否する。

### 比較で判明したワイヤの詳細 3 つ

1. **JMAP ルートの 401 は `Bearer realm="jmap"`**。
   リレー自身のハンドラは `Basic realm="biset"`。**同じ 401 で文字列が違う**
2. **エラー応答のスロット 0 はメソッド名**（`"error"` ではない）。
   `error_response` は答えているメソッド名で呼ばれる。
   **スロット 0 で失敗を判定するクライアントは永遠に判定できない**
3. `accountNotFound` は `serverFail` として出る。
   **型ではなく description が区別を運んでいる**

### 変異 2 件が通り抜けて、ハーネスについて 2 つ分かった

- `/jmap/api/` への GET は**メソッドより先に credential** で弾かれる。
  つまり**未認証の GET はメソッド判定について何も言っていない**。
  認証付き GET が要る
- **デフォルト inbox の書き込みを消しても `server_interop` は緑**だった。
  このスイートは **oracle と data ディレクトリを共有**していて、
  oracle が既に `mailboxes.json` を書いているから。
  **書き込み自体が観測できるのは新しいディレクトリだけ**なので単体テストにした

### 間欠failure を「再実行して緑」にせず、原因を潰した

`free_port` はポートを試して**解放してから**渡す。
その隙間に**並行して走る別のテストバイナリ**が同じポートを取れる。
結果、connect が成功しても**別の oracle** に繋がっていた —
seed していないディレクトリの oracle に。

症状は「setup token が無い」。

readiness の判定を、
**その子プロセス自身が自分の data ディレクトリに書くファイル**に変えた。
これは他プロセスのものではありえない。**6 回連続で green** を確認。

---

## 2026-07-30 — M6s（セッション・デバイス・ストレージ・管理系の配線）完了 (`8d96b10`)

さらに 7 ルートが 501 から外れた。テスト 627 件（34 スイート）green。

### `/account/devices` は 1 パターン 3 メソッド、そして分け方が要点

| メソッド | 認証 |
|---|---|
| POST（新デバイスの vouch） | **無し** — vouch 署名が証明。コールドリカバリの入口 |
| GET / DELETE | 必要 — 既存アカウントに作用する |

パターンを分けると `gomux.rs` 冒頭の本番障害になるので、
**1 ルートのままメソッドで分岐**する。

### anchor が要る vouch は 503 を返す

anchor クライアント自体が別マイルストーンなので、
黙って受理せず **503**（Go が「anchor に到達できない」時に使うのと同じ）。

**到達できない anchor と、まだ作っていない anchor は、
クライアントから見て同じ**。それが正直な答え。

### `/account/storage/purge-messages` は未配線のまま

メモリ上の Store（起動シーケンスのステップ 11）が要る。
**名前を挙げて 501 を assert** した — 気づかれるのを待たない。

### 管理系の一覧は「バイト比較」ではなく「数字ごとの比較」

アカウントの**集合が意図的に違う**（SPEC §11.16、oracle はドメインレジストリを
アカウントとして報告する）ので、
**実在する各アカウントの件数とバイト数が一致すること**を確認する形にした。

### interop が届かない場所は、単体で書くか、書かないか

`/admin/accounts/<address>` は bearer ガードで閉じている（§11.13）ので、
**interop からは一切到達しない**。

つまり「最後の `@` で分割する」（Go の `LastIndex`）は
**単体テストで書かなければ、どこにも検証が無い**。
localpart に `@` を含むアドレスを含めて書いた。
最初の `@` で分割する変異は、このテストが無いときは**黙って通っていた**。

---

## 2026-07-30 — M6r（WKD・オンボーディング・連絡先の配線）完了 (`a5b98d0`)

### 成果物

11 ルートが 501 から実際の応答になった:
WKD ディレクトリ、`/pgp/*` 3 本、`/auth/{envelope,signup}`、`/contacts` 2 本。

テスト 623 件（34 スイート）green。

### 比較にヘッダを足した

per-route の CORS リストは**ラッパとも互いとも違う**ので、
ここが**推測が見逃される場所**。M6q の重ね順バグが隠れていたのもここ。

ヘッダ比較を入れた結果、
「`/pgp/pubkey` のメソッドリストを変える」「`/pgp/peerkey` に
隣のルートと同じ `WWW-Authenticate` を付ける」変異が**落ちるようになった**。
入れる前は通っていた。

### 隣と違う挙動を 2 つ明記した

- **`/pgp/peerkey` は `WWW-Authenticate` を付けない 401**。
  同じ認証を要求する他のルートは全部付ける
- **`/auth/envelope` の not-found は mux の `404 page not found`**。
  ハンドラ独自のボディではない（Go が `http.NotFound` を呼んでいる）

### `json_response` と `json_response_raw` を分けた

`json.NewEncoder` は改行を足す。しかし
**envelope と秘密鍵 blob は「保存したバイト列をそのまま返す」**ので足さない。
アップロードしたものと 1 バイトも違ってはいけない。

### 自分の古い期待値が 2 つ

1. 単体テストが `/contacts` を「未配線ルートの例」に使い続けていた（今配線した）
2. テストファイルに**冗長なテスト**が残っていた —
   seed 無しで、より良い seed 付きテスト 2 本と重複していた

どちらもこの回で気づいて統合・削除した。
**配線が進むたびに「未配線」を前提にしたテストが古くなる**ので、
未配線ルートの権威あるリストは `server_interop` 側 1 箇所に集約した。

### python での一括編集を 1 度失敗した

置換が二重に当たってファイルが壊れた。
`git checkout` して**1 パスでやり直した**。
以後、複数ブロックを差し込むときは
**適用回数を assert してから書く**。

---

## 2026-07-30 — M6q（HTTP サーバ配線）完了 (`293d65b`)

### 成果物

- `jmapsmtp/src/server.rs` — axum を**トランスポートとしてのみ**使い、
  ルーティングは `GoMux`
- `jmapsmtp/src/main.rs` — SPEC §2 の順序で起動シーケンス
- `server_interop` — **移植側のサーバを oracle の隣で起動して比較**

テスト 621 件（34 スイート）green。

### これまでと違い、「関数」ではなく「サーバ」を比較した

同じ data ディレクトリの上で 2 つのサーバを立て、
**ワイヤ上のバイト列**を比べる。

**即座に 4 件の差異が出た。どれもソースを読んでも分からなかったもの:**

1. **`/relay-info` は Go の `map[string]string`** なので
   `encoding/json` が**キーをソートする**。
   struct だと宣言順になる（`{"label",…}` vs `{"color",…}`）
2. **全 JSON レスポンスが `json.NewEncoder` 経由**なので**末尾に改行**が付く
3. **`http.Redirect` は HTML ボディを書く**:
   `<a href="…">Temporary Redirect</a>.\n\n`
   （`Fprintln` が、既に改行で終わる文字列にもう 1 つ足すので 2 つ）
4. **CORS の重ね順が逆だった。**
   Go のラッパは**ハンドラを呼ぶ前に**ヘッダを書くので、
   ハンドラ自身の `Set` が**上書きする**。
   `/relay-info` は `GET, OPTIONS` を返す。
   ラッパを dispatch の**後**に適用すると、
   **全ルートの個別リストが消える**

**4 番は自分の単体テストが「逆の挙動」を assert して通っていた。**
サーバ同士を並べて初めて分かった。

### 未配線ルートは 501 を返す

404 や空の 200 にすると、difftest が**偶然一致して通ってしまう**。
501 なら差異として出る。
さらに interop に**未配線ルートを名前で列挙**したので、
1 つ配線するたびに**行を消す**ことになる。

### bearer トークンを state に移した

`env::var` を毎リクエスト呼ぶのをやめ、**起動時に 1 回**読む
（Go もルート登録時に読んでいる）。

理由はテスト容易性 — クレートが `unsafe` を禁じているので
テストから環境変数を書き換えられない。
結果として **`METRICS_TOKEN` が `/admin/*` を開けないこと**を
テストできるようになった。スクレイパのトークンが管理面に届いてはいけない。

### テスト件数の数え方を間違えていた

`awk '{if($3=="FAILED")…}'` が効いていなかった（実際は `FAILED.` で終端）。
**失敗 2 件を 0 件と表示していた。**

さらに `just test` は**最初に失敗したスイートで止まる**ので、
合計が小さくなる。以前「563 と 602 で揺れた」のもこれ。

justfile に「**テスト数だけでなくスイート数も見ろ**」と書いた。

---

## 2026-07-30 — M6p（`/setup` ページ）完了 (`310b299`)

### 成果物

- `jmapsmtp/src/setup_page.{rs,html}` — テンプレートを**逐語コピー**
- `setup_interop` に**バイト比較**を追加

テスト 602 件（33 スイート）green。

### これは「ビュー」ではなく「凍結された暗号成果物」

ページ内の JS が**ブラウザで envelope を組み立てる**
（Argon2id + AES-GCM + HKDF）。リレーはパスワードも master secret も見ない。

つまり **JS が `cryptenv` クレートとバイト単位で同じ材料を導出しなければ、
このページで作ったアカウントはログインできない**。

だから書き直さず、Go ソースから**逐語抽出して include** した。

置換は Go の `fmt.Sprintf` 準拠（`%%` を 1 パスで畳む）。
2 回の独立した replace だと `%%s` を誤る。このページの CSS が `100%%` を使っている。

### 最初のテストは推測で書いて 3 件落ちた

`enc/v1` があると思っていたが**無かった** —
設定ページは auth token しか導出しない（暗号鍵は実際に暗号化する側の仕事）。
KDF も `iterations: 3` ではなく `iterations: KDF.t` という定数参照だった。

推測を消して、**実際に書いてあるもの**を固定し直した。
「`enc/v1` が**無いこと**」も assert に含めた。

### ハーネスのバグが 2 つ出た（どちらも今後の結果を歪めていた）

**1. chunked レスポンスを復号していなかった。**
Go は書き込みバッファを超えると chunked に切り替える。
これまでの比較はたまたま全部小さくて気づかなかった。
`/setup` は `ea7\r\n<内容>\r\n0\r\n\r\n` で届いた。

**2. `free_port()` がポート 0 に bind して listener を落としていた。**
`cargo test` の並列実行で 2 つのテストが**同じ ephemeral ポート**を掴み、
片方が**もう一方の oracle**と喋っていた。
seed を見ていないリレーに、持っていないトークンを尋ねる形。

**症状は「トークンのバグに見える 401」**で、
`--test-threads=1` だと全部通った。
プロセス毎のカウンタからポートを配る方式に変更。

### テスト件数が 563 と 602 で揺れた件

ビルド途中の実行が混ざっていただけ（部分実行）。
3 回連続で **602 / 33 スイート / 0 失敗 / 0 ignored** を確認して確定させた。
**数が揺れたら、揺れの理由を説明できるまで信用しない。**

---

## 2026-07-30 — M6o（アクティビティログ）完了 (`e266c68`)

### 成果物

- `jmapserver/src/activity.rs` — アカウント毎の JSONL ログ
- `activity_interop` — oracle と比較

テスト 591 件 green。

### 型そのものが 2 つの性質を強制している

**1. メタデータのみ。**
peer / kind / サイズ / 結果は記録するが、**本文も件名も記録しない**。
これが「管理 API で公開してよい」根拠。

全 8 フィールドの行を**文字列リテラルとして assert** した。
9 個目を足すには**このリテラルを書き換える必要がある** —
件名フィールドが「ついでに」入り込まないように。
`note` は唯一の自由記述で、doc コメントに**何を入れてはいけないか**を書いた。

**2. 有界であって、アーカイブではない。**
2 MiB を超えると `activity.log.1` に 1 世代だけ退避（上書き）。

**リモートの活動で追記されるアカウント毎ログが無制限**だと、
外部からディスクを埋める手段になる。

### テスト自体について 2 つ記録した

**フィールド順のチェックは `serde_json::Value` を通せない。**
`Value` は BTreeMap なので、パースしてキーを読み直すと**ソートされる**。
固定したい宣言順が見えなくなる。文字列比較にして、理由をテストに書いた。
（M6e の `extract_text_headers` と同じ事実）

**順序の変異 2 件が interop を通り抜けた。**
テストが **oracle の出力**に対して assert していて、
**移植側の reader を一度も呼んでいなかった**から。

並びを**左右に並べて比較**する形に直した（limit 付きも含む）。

**同じ間違いが 3 例目**（M6j, M6n, ここ）。ルールを明文化した:

> **interop テストは、移植側のどの関数を実行しているかを名指しできること。**

---

## 2026-07-30 — M6n（管理系とメトリクス）完了 (`cea246d`)

### 成果物

- `jmapserver/src/admin.rs` — `/admin/accounts` とメトリクス収集
- `admin_interop` — oracle と比較

テスト 576 件 green。

### Go 実装の中で、2 つの答えが食い違っていた（SPEC §11.16）

`ListProvisioned`（`/admin/accounts` の元）は `peers` を除外するが
**`_domains` は除外しない**。metrics のコレクタは**どちらも除外しない**。

oracle で実測:

```
GET /admin/accounts
  → "byo.test@_domains"   ← 存在しないアドレス
  → "alice@a.test"

GET /metrics
  biset_accounts{domain="_domains"} 1
  biset_accounts{domain="a.test"}   2   ← peers を 1 アカウントと数えている
```

**同じドメインについて、一覧は 1、メトリクスは 2。**

影響:

- アカウント数の監視が水増しされる
  （水増し量 = peer 鍵のあるドメイン数 + 登録済みカスタムドメイン数）
- `ListProvisioned` は **`anchor.Drain` の元でもある**ので、
  存在しないアドレスを release しようとする

Rust は両方除外し、**metrics も同じ walk を使う**ので合計が必ず一致する。

> 運用者にとっては、どちらかが間違っていることより
> **2 つが食い違っていて突き合わせられない**ことのほうが悪い。

### 変異が 2 件通り抜けて、M6j と同じ形だと分かった

ディスクメトリクスを「アカウントの合計」に書き換えても、
`build_info` を 0 にしても、**interop は緑のまま**だった。

理由: interop が `dir_bytes` を**直接**呼んで比較していた。
`collect()`（実際にエンドポイントを提供する関数）を通していなかった。

**M6j と同じ**（あちらは移植側の関数を一度も呼んでいなかった）。
どちらも「**比較対象が、実際に動く経路ではなかった**」。

修正して、`collect()` を通し、
さらに「移植側が出す全メトリクス名が oracle にも存在すること」も確認するようにした。

以後、interop を書くときは
**「エンドポイントが呼ぶのはどの関数か」を先に確認**する。

---

## 2026-07-30 — M6m（DID 起点の連絡先キャッシュ）完了 (`6f348a6`)

### 成果物

- `jmapserver/src/contacts.rs` — JSContact (RFC 9553) Card の永続化
- `contacts_interop` — **両方向**（Go が書く→Rust が読む / その逆）+ バイト比較

テスト 561 件 green。

### Card が束ねているもの

**DID は `cryptoKeys` に入る** — DID は構造上 URI なので拡張プロパティが要らない。
Card が記録しているのは
**「このアドレスは、最後に確認した時点でこの DID のものだった」**。

**アドレスはアイデンティティ間を移動しうる**ので、
DID の付いていないキャッシュ済みアドレスは、キャッシュが無いより悪い。
テストは両方向で書いた:

- 同じ DID のままアドレスが変わる
- 同じアドレスのまま DID が変わる ← クライアントが
  「今は別人と話している」と分かる必要がある

did:webvh はコロンとパスセグメントごと**verbatim で往復**する。
分解して再構成する実装が壊れる形。

### upsert は「マージ」かつ「位置を保つ」

- 丸ごと置換だと、**1 件の write-through が他の全連絡先を失う機会**になる
- 並び替えると、**2 実装が互いのファイルを churn させる**

### 現実的な入力の穴が 4 例目 — これが一番はっきりした

`go_json` を `serde_json` に差し替えても**何も落ちなかった**。

理由: **テストの Card に `&` `<` `>` が 1 つも入っていなかった**。
Go は既定でこれらを HTML エスケープする（SPEC §4）ので、
**含まれていなければ両者のバイト列は一致してしまう**。
つまりバイト比較テストが、エスケープを再現しているかどうかに無関係だった。

修正: fixture に**クエリ文字列付きのサービスエンドポイント URI**を入れた
（現実的な形）。さらに単体・interop の両方で
**エスケープ後の形が実際に含まれていること**を assert した —
fixture が再び牙を失わないように。

| | 欠けていた入力 |
|---|---|
| §11.14 | HTML パート |
| §11.15 | 別々の鍵 fixture |
| M6l | `messages/` 内のディレクトリ |
| ここ | **エスケープ対象文字** |

4 例とも「テストが弱い」のではなく
**「テストの入力が、その性質を観測できる形になっていない」**。
以後、バイト比較テストには
**「比較が意味を持つために入力が満たすべき条件」を assert で書く**。

---

## 2026-07-30 — M6l（ストレージ透明性エンドポイント）完了 (`07b17eb`)

### 成果物

- `jmapserver/src/storage.rs` — `/account/storage{,/messages,/export,/purge-messages}`
- `storage_interop` — oracle と比較

テスト 542 件 green。

### 削除するルートを公開できる理由

**リクエストにメールアドレスが一切現れない。** 対象は credential からのみ導出される。
だから「他人のアカウントを purge する」経路が原理的に無い。

テストでは**クエリとボディの両方に他人のアドレスを入れて**、
無視されることを確認した。

### purge のテストは「消すこと」ではなく「他を消さないこと」

アカウントディレクトリを**前後で丸ごと取って差分を比較**する。
範囲が広がったら、**広がって消したファイル名がテストに出る**。

`PURGE_MUST_NOT_TOUCH` を明示リストにした。
「messages/ だけ触る」という含意に任せない —
**範囲を広げるには「広げてはいけない理由」が書かれた行を消す必要がある**ようにした。

載っているものは全部、
**第二の複製が存在しない credential**（`envelope.json` / `privkey.enc` / `devices/`）か、
**アカウント唯一のメールボックス**。

### `messages/` の中にディレクトリを置いたら 2 つ分かった

現行レイアウトでは想定されないが、
**「メッセージ数」と「エントリ数」を区別する唯一のケース**:

1. 一覧は**数えないしサイズにも足さない**（両実装とも）
2. purge は**再帰しない**（`messages/stray/inner.json` は残る）

2 番目は自分の予想が外れた。しかし**一貫している** —
一覧が報告していないバイトを purge が消したら、
「報告していないものを消した」ことになる。

### 現実的な入力の話が 3 例目

| | 欠けていた入力 |
|---|---|
| §11.14 | HTML パート |
| §11.15 | 別々の鍵 fixture |
| ここ | `messages/` 内のディレクトリ |

**3 回続いたので、seed は「整った例」ではなく「気まずい例」を意図的に入れる**
方針に切り替えた。今回はそれで変異 1 件（ディレクトリをメッセージとして数える）
が捕まるようになった。

---

## 2026-07-30 — M6k（カスタムドメインとアカウント削除）完了 (`1045ad7`)

### 成果物

- `jmapsmtp/src/customdomain.rs` — BYO ドメインの検証フローと `/account/delete`
- `customdomain_interop` — oracle と比較

テスト 520 件 green。

### 設計の要: 検証済みドメインは「開放」にならない

登録は `allow_provision: false` +
**その都度再発行される provision_secret** を書く。

`allow_provision: true` にしてしまうと、
**一度登録すれば、以後は誰でも他人のドメイン配下にアカウントを作れる**。
所有権の再証明が二度と要らなくなる。

`/domain/add` は登録済みドメインに対しても**毎回 TXT を確認する**。
「過去に所有していた」ではなく「**今 DNS を握っている**」がゲート。

### 2 つのトークンは衝突してはいけない

どちらも同じ secret から HMAC で導出される:

| | 用途 | 公開性 |
|---|---|---|
| `verify_token` | DNS に公開する TXT | **公開** |
| `provision_secret` | アカウント作成のゲート | **秘密** |

**同一になると、所有権を証明すること自体がゲートを公開する。**
テストで直接比較し、さらに
「所有権証明前のレスポンスに secret が含まれないこと」も確認した。

### 412 であって 400 ではない

リクエストは正しく、**世界の側の前提**が満たされていない。
DNS 伝播があるので「まだ」は普通のケース。
クライアントはリクエストを書き直すのではなく**再試行すべき**。

### 設定済みアカウントは自己削除できない

`config.json` が名前を持っているので、
消しても**次回起動で戻ってくる**（データだけ失われる）。

### `valid_username` と同じ形が 2 例目

`valid_custom_domain` は大文字を拒否するが、
**両エンドポイントとも呼ぶ前に小文字化している**ので
`Example.com` は成功して `example.com` を登録する。

oracle は**小文字化後のドメインのトークン**を返す。
これはクライアントにとって正しい —
**実際に登録されるドメインのレコードを公開できる**から。

M6f と同じパターンなので、
**doc コメントに「述語は厳しく見えるが、呼び出し側が正規化している」形として明記**した。
次に述語を書くときに同じ目で見るため。

### 変異テスト 6 件

うち 1 件（TXT の前方一致を許す）は**単体のみ**で落ちる —
`/domain/add` は実際の DNS を引くので oracle 経由では駆動できない。
分けて記録した。

---

## 2026-07-30 — M6j（オンボーディング）完了 (`3791c3e`)

### 成果物

- `jmapsmtp/src/setup.rs` — `/auth/envelope`、`/auth/signup`、`/relay-info`
- `setup_interop` — oracle と比較

テスト 494 件 green。

### 「一見おかしい」2 点を冒頭に書いた

**1. `GET /auth/envelope` は公開。**
envelope は Argon2id(t=3, m=64MiB, p=4) + AES-GCM で包まれた master secret なので、
パスワード無しでは無価値。そしてクライアントは
**credential を持つ*前*に**これを必要とする（credential はこれを解いて導出される）。

漏れるのは「どのアドレスが envelope 付きアカウントを持つか」。
**フローに内在するものであって、うっかりではない。** Go も同じ取引をしている。
発見される前に記録した。

**2. `POST /auth/signup` は意図的に非冪等。**
envelope を既に持つアカウントは拒否する。
そうしないと**リプレイで別の envelope を被せて、そのアカウントを奪える**。
ローテーションは現行 credential が要る `PUT /auth/envelope` 側。

### トークンは envelope を書いた「後」に焼く

逆順だと**書き込み失敗時に、二度と claim できないアカウント**が残る。
不正なボディを送ってから再試行が成功することをテストで固定した。

### `/relay-info` は oracle に聞いて 2 フィールド足りなかった

- `type`（`"mail"`）— クライアントが**接続先リレーの種別を判定**できる。
  自分の config の URL と一致するかで推測していたのを置き換えるため
- `domain` — **新規アカウントが実際に着地するドメイン**。
  リレーのホスト名とは限らない。
  `username@<ホスト名>` をプレビューする実装は、両者が違うと常に間違っていた

`domain` は開放ドメインが無ければ**省略**（null ではない）。
空文字を出すより「プレビューできないと分かっている」ほうがよい。

### interop が oracle しか叩いていなかった

`signup` を変異させても**緑のまま**だった。
テストが oracle の応答だけを見ていて、**移植側の関数を一度も呼んでいなかった**から。

修正: **oracle 自身の data ディレクトリ**に対して、
oracle が触っていない 2 つ目のアカウントで**移植側の `signup` を走らせ**、
結果を **oracle の HTTP 経由で確認**する。
つまり「移植側が書いたものを、動いている Go バイナリが読み返す」形。

これで変異が落ちるようになった。

**これは前 4 件とは別の穴** — 入力が現実を代表していないのではなく、
**比較対象が片側しか無かった**。相互運用テストと名乗りながら
一方向しか見ていないパターンは他にもありうるので、以後は
「このテストで移植側のどの関数が実行されるか」を明示的に確認する。

---

## 2026-07-30 — M6i（WKD と PGP 鍵配布）完了 (`6ad1881`)

### 成果物

- `jmapsmtp/src/wkd.rs` — WKD と `/pgp/{pubkey,privkey,peerkey}`
- `wkd_interop` — oracle と比較（**リレー鍵あり / 無しで 2 回起動**）
- `xtask/fixtures/pgp-public-relay.asc` — **2 本目の鍵**（後述）

テスト 465 件 green。

### 【重大】WKD で大文字混じりの `l=` がリレーの鍵を返す（SPEC §11.15）

`wkdHash` は小文字化するのでハッシュ比較は case-insensitive。
しかし直後のアカウント検索は `domCfg.Accounts[localpart]` で**生のパラメータ**。

`?l=Alice` は:
1. ハッシュ比較を**通る**（「これは alice」）
2. アカウント検索で**外れる**
3. **リレー全体の鍵にフォールバックする**

oracle で実測:

| リクエスト | 返る鍵 |
|---|---|
| `?l=alice` | alice の鍵（2294 B） |
| `?l=Alice` | **リレーの鍵**（2359 B） |

アドレス帳に `Alice@a.test` を持つ送信者は、
**リレーが持っていて alice が持っていない鍵**で暗号化する。

- リレーはそのメールを**読める**
- alice は**読めない**
- 送信者は E2E だと思っている
- **どこにもエラーが出ない**

Rust はアカウント検索の前に小文字化した。
安全な理由: アカウントキーは常に小文字（provision が小文字化する）ので、
**ハッシュが既に特定した 1 アカウント**にしか当たらない。

### なぜ今まで見えなかったか — fixture を兼用していた

リレー鍵とアカウント鍵に**同じファイル**を使っていた。
すると「alice の鍵が返った」という assert が**恒真**になる。

鍵を 2 本に分けた瞬間に露見した。
interop テストのコメントに
**「1 つの fixture を兼用すると、ここの assert は全部無意味になる」**
と理由を書き残した。

**§11.14（HTML パート欠落）と同じ失敗モード** —
「テストの入力が現実を代表していない」。これで 2 例目。
**fixture の使い回しは、それ自体がテストの弱点になりうる。**

### interop を 2 回起動する理由

リレー鍵の有無で**すべての WKD 検索の答えが変わる**。
無いと不一致は常に 404 なので、
**「フォールバックを拒否する枝」に到達できない**。
その枝がまさに間違った鍵を返す場所だった。

（`BISET_PGP_KEY` は config ではなく**環境変数**なので、
ハーネスに env を渡す機構を足した）

### 4 ルートの露出度を表にした

混同すると秘密鍵が公開されるので、モジュール冒頭に表で置いた:

| ルート | 認証 | 理由 |
|---|---|---|
| `openpgpkey/policy` | 無し | WKD のマーカー。**空の 200 が答え** |
| `openpgpkey/hu/<hash>` | 無し | **設計上の公開ディレクトリ** |
| `/pgp/pubkey` PUT | アカウント | 自分の鍵を書く |
| `/pgp/privkey` | アカウント | クライアント側暗号化済みでも**秘密鍵** |
| `/pgp/peerkey` | アカウント | メールから集めた Autocrypt 鍵 |

### 気づいた点

- 公開鍵は**アップロードされた armored バイトをそのまま保存**する。
  再シリアライズしないので往復がバイト一致する
- WKD は**バイナリパケット**で返す（armor ではない）。ディレクトリの規約
- **秘密鍵 blob は検証しない**。復号できないものをパースできるふりをしないため
- peer 鍵は**ドメイン単位**。同一ドメインの 2 アカウントが
  同じ相手の鍵を別々に再発見する必要は無い

---

## 2026-07-30 — M6h（デバイス / セッションエンドポイント）完了 (`bad538c`)

### 成果物

- `jmapsmtp/src/devices.rs` — `POST /account/session` と `/account/devices`
- `devices_interop` — oracle の実エンドポイントに投げて比較

テスト 433 件 green。

### この 2 つは credential を取らない（意図的）

移植中に取り違えないよう明記した:

| | 認証 | 理由 |
|---|---|---|
| `POST /account/session` | **無し** | デバイス署名が credential そのもの |
| `POST /account/devices` | **無し** | vouch 署名が証明。**完全なコールドリカバリ**（ニーモニックのみ・新規インストール・既存セッション無し）の入口 |
| `GET`/`DELETE /account/devices` | `authenticate()` | 既存アカウントに作用するので credential が要る |

3 つで**ルートパターンは 1 本**。分割すると `gomux.rs` 冒頭の本番障害になる。

### 誰でも叩けるので、応答が security boundary になる

**ログイン失敗のメッセージを 1 種類に保つこと**が要件。
不明アカウント / 未登録デバイス / 鍵違い / 期限切れ / 未来の 5 ケースについて、
**oracle の応答を集合に入れてサイズ 1 を要求**した。
そうでなければ「どのアカウントとデバイスが存在するか」の名簿になる。

もう 1 つ: **did:webvh の vouch は anchor 無しで 503、401 ではない。**
vouch は正しいかもしれず、**このリレーが判断できない**だけ。
401 だとクライアントに「署名が間違っている」と伝えてしまう。

### 前回の教訓を前向きに適用した

拒否ケースの署名を、ゴミバイトではなく
**「間違った statement に対する本物の ed25519 署名」**にした
（別ラベル / 別デバイス / 別鍵）。

これは 2 コミット前に `devicekeys_interop` で見つけた弱点そのもの
（署名が合わないから通っていた）。**同じ穴を新しいテストで掘らないよう**、
最初からこの形で書いた。

### 変異テスト 5 件

| 変異 | 結果 |
|---|---|
| webvh の 503 を 401 に | interop + 単体で落ちる |
| ログイン失敗を区別する | interop で落ちる（集合サイズ 2） |
| 存在確認を `auth_token_hash` だけに | **単体のみ**で落ちる（HTTP 経路が未配線） |
| TTL を 1 日に | interop + 単体で落ちる |
| vouch 対象から label を外す | interop + 単体で落ちる |

3 番目を「interop で捕まえた」と書かないよう分けて記録した。

---

## 2026-07-30 — M6g（Store フック）完了 (`aa6c72d`)

### 成果物

- `jmapsmtp/src/hooks.rs` — `OnCreateEmail` / `OnSubmitEmail`（I/O を外に出した判断関数）
- `hooks_interop` — **oracle の実 JMAP API を叩いて、保存されたファイルを読む**

テスト 413 件 green。

### 【重大】封印した保存コピーに平文 HTML が残っていた（SPEC §11.14）

送信メッセージの保存コピーを封じる目的は
**リレーがユーザの送信メールを読めないようにすること**。Go はそれを達成していない。

```go
msg.HTMLBody = nil    // ← 「参照」は消すが「値」は残る
```

oracle が保存したファイルを実際に読んだら:

```json
"bodyValues":{
  "1":{"value":"-----BEGIN PGP MESSAGE-----…"},
  "2":{"value":"<p>the secret plaintext</p>"}      ← 平文
}
"textBody":[{"partId":"1",…}]                       ← htmlBody は消えている
```

**参照されていない part `2` に平文が残る。**
リッチテキストのクライアントは HTML 代替を必ず送るので、
**実運用では大半のメッセージが該当する。**

Rust は参照と一緒に値も消す。Go が始めた処理を完了させただけで意図は変えていない。

**範囲は意図的に広げていない**: 添付の body value はそのまま保存される。
添付は「封印した本文の別レンダリング」ではないので、
扱いを決めるとクライアントが開けるものが変わる。SPEC に記録して止めた。

### 見つかった経緯 — 変異が通り抜けたから

`msg.html_body.clear()` を消しても **interop テストは緑だった**。
理由は単純で、**テストのメッセージに HTML パートが無かった**。

実運用の形（HTML 代替あり）にしたら、
**弱いテストと、その裏の Go のバグが同時に出た。**

これで 4 件目。前回書いた
「変異が通り抜けた時は、たいてい知らない挙動がある」は、
**「テストの入力が現実を代表していない」も含む**と分かった。

### 下書きはディスクに書かれない

最初のテストは下書きをディスクから読もうとして**何も無かった**。
`PutPending` はメモリのみ（`store.go` に明記）。
つまり**未送信の下書きは再起動で消える。**

回避せず固定した。これは
**create フックが Message-ID を先に採番できる理由**でもある —
下書きはクライアントの状態で、リレーは create と submit の間だけ持つ。

### 変異テスト 7 件、1 件はコンパイルが拒否した

`let sealed = &mut msg.body_values;`（クローンせず in-place）は
**借用チェッカが通さない**。テストが捕まえたのではなく、型システムが拒否した。
これはこれで良い結果だが、**「テストで捕まえた」と書かないよう**分けて記録した。

（この変異が意味を持つのは、封印が送信コピーにも及ぶと
**鍵を持たない相手に暗号文を送る**ことになるから）

---

## 2026-07-29 — M6f（provision = DID がアカウントになる場所）完了 (`cf7ed67`)

### 成果物

- `jmapsmtp/src/provision.rs` — HTTP も I/O も含まない判断ロジック
- `provision_interop` — **oracle の実エンドポイントに 14 リクエスト**投げて突き合わせ

テスト 387 件 green。

### なぜ判断ロジックを HTTP から分けたか

`/account/provision` の答えは
**ビルド（anchor 有無）× config × DID のメソッド × ディスクの状態**で決まる。
ハンドラの中に埋めると、この直積のうちテストから到達できない枝が必ず出る。

分けた結果、13 種の拒否すべてに単体テストと差分テストが付いた。

### 記録した挙動（SPEC §10-A）

| | 理由 |
|---|---|
| **デバイス鍵が credential そのもの** | このフローは `auth_token_hash` を一切書かない。vouch 検証→書き込みがアカウント登録より先なので「作ってから後でデバイス追加」の隙間が無い |
| did:dht は **anchor があってもローカル検証** | 識別子が鍵を持っているので往復に意味が無い |
| did:webvh は anchorless で**作れない** | SCID は genesis log entry のハッシュ。照合相手が存在しない |
| 名前の重複は**両方の credential 形状**で見る | `auth_token_hash` だけ見ると、このフローで作られたアカウントを他人に渡す |
| 存在しないドメイン指定は**開放ドメインに落とさない** | クライアントが頼んでいない場所に、別ドメイン向けに署名された名前で作られる |
| 空の `provision_secret` は空の提出と**一致してはいけない** | 素の `!=` だとそうなる。特権ドメインが作成可能になる |

did:webvh の拒否は `DeviceVouchRejected` と**別のメッセージ**にした。
「vouch は正しい、*リレー*が検証できない」はクライアントが対処できる情報だから。

### oracle は anchorless で動かした

これが**退化した設定ではなく、一番面白い設定**。
2 つの DID メソッドが分岐するのはここだけ。
anchored oracle だと本物の anchor に接続しにいってしまう。

各ケースに**専用の did:dht アイデンティティと本物の vouch 署名**を作った
（`seed` を変えた ed25519 鍵から `devkey:...` を署名）。
先のケースが作ったアカウントが後のケースの答えを変えないように、
username も全部別にした。

### 変異テストが 1 件通り抜けて、未検証の挙動が見つかった

`valid_username` の「先頭は小文字か数字」を `is_ascii_alphanumeric` に緩めても
**何も落ちなかった**。理由: `validate` が**先に小文字化している**ので
その枝に到達しない。

つまり **大文字 username は拒否ではなく成功する**（`Alice` → `alice`）。
これは今までどのテストも見ていなかった実際の挙動。

固定したもの:

1. `valid_username("Alice") == false`（述語単体としては拒否する）
2. `validate` は trim + 小文字化してから聞くので `"  Alice  "` は **Ok**
3. 差分テストで **`open.test/nora` が作られ、`open.test/  Nora  ` は無い**

これで 3 件目。**変異が通り抜けた時は、たいてい「テストが弱い」のではなく
「知らない挙動がある」**というパターンが見えてきた。

---

## 2026-07-29 — アイデンティティは DID である（`dedbd37`）

biset 本体を読んだ（https://github.com/yno9/biset @ `6030a0b` → `~/biset/`）。
**リレー側のコードから推測せず、クライアントの `src/did/` を読んだ。**

### 判明したこと（SPEC §10-A にした）

**このリレーにとってユーザの同一性はアドレスではなく DID。**
アドレスはその DID が claim しているルーティングラベルにすぎない。

```
DID（ルート鍵）
  └─ devkey:<did>:<devicePubKey>:<label>:<ts>   ← ルート鍵が署名
       └─ device key
            └─ session:<did>:<devicePubKey>:<ts>  ← デバイス鍵が署名
                 └─ session token
```

署名対象文字列は biset `src/did/devicebind.ts` の
`vouchStatement` / `sessionLoginStatement` と**バイト一致していた**（確認済み）。

### DID はディスクに保存しない（意図的）

`POST /account/provision` は DID を**必須**にする（無ければ 400）のに、
**リレーはどこにも DID を保存しない。**

> keeping a second copy is what let this one drift out of step with the
> registry — `provision.go`

**移植で「便利だから」DID ファイルを足してはいけない。** 一度やって壊れている。

### did:dht と did:webvh は非対称 — SCID は鍵ではない

| | 識別子の中身 | ローカル検証 |
|---|---|---|
| `did:dht:<zbase32(pubkey)>` | **ルート公開鍵そのもの** | できる |
| `did:webvh:<SCID>:<domain>:<path…>` | **genesis log entry のハッシュ** | **できない** |

SCID = `base58btc(multihash(JCS(genesis log entry), sha256))`
（biset `src/did/webvh/scid.ts`、base58btc 46 文字）。
**自己証明しているのは DID document log であって署名鍵ではない。**
現在の鍵は解決済み log の中にしか無い。

だから anchorless リレーは **did:webvh アカウントを作れない**。

### 敵対的テストを書いた

**SCID を鍵として扱うと、誰でも任意の webvh アイデンティティに対して
デバイス vouch を偽造できる。**

そこで「webvh の形をしていて、SCID スロットに**本物の zbase32 鍵**が入っていて、
**正しい署名まで付いている**」ケースを拒否することを固定した。
同じ鍵を `did:dht:` にすると通る — つまり拒否の理由が
**接頭辞チェックだけであること**まで確認している。

> SCID が 46 文字で 32 バイトにデコードできないのは**偶然であって防壁ではない。**

### ついでに、既存テストの実質的な穴を潰した

`did_dht_vouches_are_judged_identically` は
**署名を 1 回作って全行で再利用**していた。
つまり拒否ケースは「DID が拒否されたから」ではなく
**「署名が合わなかったから」**通っていた。

`did_dht_root_key` に `did:webvh:` を受け入れさせる変異を入れても**緑のまま**だった。
行ごとに署名し直すよう直したら、その変異で落ちるようになった。

**M6e の `out.sort()` と同じ種類の穴** — 2 件目。
「赤にできないテストは無価値」を、既存テストにも遡って当てる必要がある。

---

## 2026-07-29 — M6e（handler の識別子とアカウント配送表）完了 (`b0e436a`)

### 成果物

- `jmapsmtp/src/handler.rs` — `makeStore` の識別子・容量計算・エイリアス表の部分
- `handler_interop` — **oracle が起動時にディスクへ書いたファイル**とバイト比較

テスト 356 件 green。

### Go ヘルパを書かずに済んだ

`defaultInbox` / `makeMailboxID` / `writeDKIMDNSFile` は
`package main` の非公開関数なのでリンクできない。しかし**通常起動すれば出力がディスクにある**:

```
data/a.test/alice/mailboxes.json
data/a.test/dkim-dns.txt
data/a.test/alice/setup.token
```

ヘルパを書くより強い。**実運用に存在するのはこのファイルそのもの**だから。
しかもこれらは**プロセスより長生きする** — 差異は
「アップグレード後にクライアントの mailbox id が変わる」
「移行後にメールが DKIM で落ちる」として現れる。

### 識別子の 2 つの性質

| | 理由 |
|---|---|
| mailbox id は**アドレスから導出**（採番しない） | 再起動でクライアントのキャッシュが無効化されない |
| message id は **RFC Message-ID 優先** | 再配送（リトライ、2 番目の MX）が重複ではなく上書きになる |

`/` の置換文字が 2 関数で 3 種類（`~` `_` `-`）ある。
醜いが**そのまま維持**した — 既にディスク上のファイル名に現れているので、
統一すると保存済みメッセージが全部孤児になる。

### 変異テストが 1 件、捕まえられなかった（隠さず記録した）

`extract_text_headers` の `out.sort()` を消しても**どのテストも落ちない**。
`serde_json::Map` は `preserve_order` feature が無ければ `BTreeMap` なので、
**入力が既にソート済み**だから。

つまり「ソートを固定している」と称するテストは、実際には何も固定していなかった。

対応:

1. `sort()` は**残す**。tree 内のどのクレートでも `preserve_order` を
   有効化できてしまい、その時これが Go の map 順序非決定性との唯一の防壁になる
2. コメントに「**defensive であって load-bearing ではない**」と明記
3. **前提そのものをテストにした** — `serde_json_object_iteration_is_already_ordered`。
   feature が有効化されたらこれが先に落ちて理由を名乗る

「緑のテストは、赤にできなければ無価値」を、
**赤にできないと分かった時にどうするか**まで含めて適用した形。

### 気づいた点

- `dir_size_mb` は**切り捨て**（Go と同じ）。1.9MB は 1 と読む。
  切り上げると上限直下のアカウントを拒否してしまう
- 読めないツリーは 0 として扱う。**容量チェックが送信を壊す手段になってはいけない**
- `Accounts::remove` はエイリアスも消す。残すと
  **到達できない store に解決され、そのアドレス宛のメールがエラー無く消える**

### 次

`OnCreateEmail` / `OnSubmitEmail` フック本体、そして各エンドポイント。

## 2026-07-29 — M6d（ルートテーブルと ServeMux 移植）完了 (`00e8b84`)

### 成果物

- `jmapsmtp/src/gomux.rs` — `net/http.ServeMux` のルーティング移植
- `jmapsmtp/src/routes.rs` — 全ルートを 1 箇所に（登録が重複判定を兼ねる）
- `jmapsmtp/src/bearer.rs` — 管理系のトークン検証（**Go と意図的に違う**）
- `mux_interop` — ルート表とルーティング規則を oracle と突き合わせ
- `tests/oracle_harness/` — oracle を起動して叩く共通ハーネス

テスト 333 件 green（anchor / noanchor 両方で clippy clean）。

### axum を使わなかった理由 3 つ

1. **同じパターンの二重登録が panic する**。これが `route_registration_test.go` の
   守っている本番障害そのもの。axum は黙って受け入れるので、
   **移植すると「クラッシュを失い、バグを残す」**
2. **サブツリーパターン**。`/jmap/api/` は `/jmap/api/何でも` に一致し、
   残りはハンドラが自分で切る。axum なら 404
3. **リダイレクト**（クエリ付きで保存）

### Go のバージョンでリダイレクトのステータスが違う

**このマシンの `go` は 1.22 で、そのソースは 301 と書いてある。
oracle は 1.26.3 でビルドされていて、実際は 307。**

```
$ curl -si .../jmap/api
HTTP/1.1 307 Temporary Redirect
Location: /jmap/api/
```

ソースを読んでいたら**全サブツリールートで誤ったリダイレクト**を返す移植に
なっていた。**動いているバイナリに聞く**方針が、また効いた。

`gomux.rs` の冒頭にこの事情を書き、
「oracle を古い toolchain で再ビルドすると `mux_interop` が落ちる。
それが正しい挙動で、**テストを緩めるのではなく、どちらを送るか決める**」
と明記した。

### oracle に聞いて、ルート表が 3 箇所直った

登録箇所を読むだけでは分からない条件が付いていた:

| ルート | 実際の条件 |
|---|---|
| `/jmap/upload/` `/jmap/download/` | ハンドラが `BlobHandler` を実装している必要。**このリレーは実装していない** |
| `/pkarr/` | anchor ビルドだけでは足りず、**`anchor_url` が非空**である必要 |

### 【重大】`ADMIN_TOKEN` 未設定で管理系が全開になる

`bearerAuth` は `if token != ""` で囲まれている。
**空なら検証を一切しない。** oracle で実測:

```
GET /admin/accounts  → 200 {"accounts":[{"address":"alice@a.test",...}]}
GET /metrics         → 200
POST /admin/drain-anchor → ハンドラ到達（400）
```

最後は **anchor 上の claim を全部 release する POST が無認証**。

Rust は空トークンを**閉じる**扱いにした（SPEC §11.13）。
理由: 「設定していない運用者は、公開を選んだのではなく、考えていない」。

**アップグレードで 1 ケース壊れる**ことは認識している —
`METRICS_TOKEN` 無しの Prometheus スクレイプが 401 になる。
それでもこちらを選んだ。破損は即座に見えて直し方は環境変数 1 つだが、
情報漏洩は見えない。「起動を拒否する」案は、
**アップグレード後に起動しなくなるほうが 401 より重い**ので却下した。

### ハーネスがビルドを取り違えていた

`--no-default-features`（= `-tags noanchor`）のテストが
**anchor 版 oracle と比較していた**。別のプログラムなので比較が成立しない。
`ORACLE_BIN` を `cfg!(feature = "anchor")` で切り替えた。
noanchor ビルドで anchor ルートを主張させると、ちゃんと落ちる。

### このテスト対で捕まえられないもの（明記した）

**oracle にあって移植側が一度も挙げていないルート**は検出できない。
`ServeMux` を HTTP 越しに列挙する方法が無いため。
両リポジトリの `HandleFunc(` / `mux.Handle(` を網羅 grep して作り、
**ルート数を assert** して、減った時に diff に出るようにした。
テストファイルにこの限界をそのまま書いた。

### 存在確認の方法（ハンドラの 404 と区別する）

ステータスコードでは判定できない —
`/.well-known/openpgpkey/hu/` は鍵が無いとき `http.NotFound` を呼ぶので、
**mux 自身の 404 とバイト単位で同一**。

サブツリーパターンを**末尾スラッシュ無しで**要求すると 307 が返る。
これは mux にしか出せない答えなので曖昧さが無い。
（`/contacts` のように末尾無しも登録済みの場合だけ、弱い判定に落とす）

### 次

M6e: handler と Store 構築、そして各エンドポイントの中身。

## 2026-07-29 — M6c（起動シーケンス）完了 (`0430c38`)

### 成果物

- `jmapsmtp/src/startup.rs` — SPEC §2 のステップ 5〜12（listener を開く前まで）
- `startup_interop` — **孤児掃除を oracle バイナリと突き合わせる**

テスト 298 件 green。

### 「順序が契約」を、順序を壊したテストで固定した

SPEC §2 に書いてあるだけでは移植で守られない。実際に**壊した側も書いた**:

```
// 誤った順序: レジストリが空のまま掃除
removed == ["data/byo.test"]   ← これが防ぎたいデータ損失
// 正しい順序
removed.is_empty()
```

検証済みカスタムドメインは `data/_domains/` にしか存在しないので、
先に掃除すると**そのドメインの全アカウントのメールが消える**。
テスト名にも `..._survives_only_because_it_was_loaded_first` と書いた。

### アカウントの定義は 2 箇所で一致していなければならない

掃除（消す側）と復旧スキャン（戻す側）が**別の定義を持つと、
どちらの向きでも再起動が破壊的になる**。片方が消し、片方が戻す。

両方 `auth_token_hash` を見る。`envelope.json` は見ない。
`the_scan_recovers_exactly_what_the_sweep_keeps` がこの一致を固定する。

### `remove_dir_all` を Go ソースの読解で済ませたくなかった

掃除はユーザのメールを消す。**oracle バイナリを実際に起動して**
生き残ったディレクトリを比較する形にした。

- Go ヘルパを書かない。**検証対象がバイナリの起動そのもの**なので、
  ヘルパを挟むと「テストと対象の間に再実装が入る」
- oracle は `argv[0]` のディレクトリから `config.json` を読むので、
  symlink を temp dir に置けばよい（`filepath.Abs` は symlink を解決しない）
- 起動完了の合図は **HTTP listener が開くこと**（ステップ 15 = 最後）

seed には**鏡像のアカウントを 2 つ**入れた —
「hash あり envelope 無し」と「envelope あり hash 無し」。
これで envelope 基準の実装は**両方向で**落ちる。実際に変異させると:

```
left:  [..., "a.test/enveloped", ...]        ← Rust が残した
right: [..., "a.test/dynamic", "byo.test/carol"] ← Go が残した
```

### 気づいた点

- setup token は**既存ファイルを再利用**する。毎回発行すると
  「オンボーディング中の再起動が、送信済みリンクを黙って無効にする」
- `peers/` はアカウントディレクトリと同じ階層にあり、
  **ドメイン全体の Autocrypt 鍵**が入っている。予約名リストで守った

---

## 2026-07-29 — M6b（config と認証層）完了 (`9006f4f`)

### 成果物

- `jmapsmtp/src/config.rs` — `config.json`（Go とスキーマ同一）
- `jmapsmtp/src/auth_env.rs` — ログインと、資格情報が載る 2 ファイル

テスト 284 件 green。

### 同梱の `config.example.json` が起動しなかった

**コピーしても起動しない**サンプルだった。2 点:

1. `anchor_url` あり + `anchor_token` 空 = `checkAnchorConfig()` が
   `log.Fatalf` する組み合わせそのもの。
   **初回起動が、操作した覚えのないフィールドの話をして死ぬ**
2. `account` のキーがフルアドレス（`you@example.com`）。
   キーは localpart なので `you@example.com@example.com` になる

サンプルは互換性の対象ではなく**ドキュメント**なので直した（SPEC §11.12）。
パーサは 1 バイトも変えていない。
`the_shipped_example_config_loads_and_validates` が、
このファイルが実際に通ることを固定する。

### 認証は「順序」が効いている

1. **セッショントークン**を先に見る。よくある経路であり、かつ**失効できる**方
2. 次に静的 `auth_token_hash`

そして「アカウントが既知か」を**ハッシュファイルを読む前に**確認する。
逆順だと、ディスクに残骸ディレクトリがある localpart
（削除済み、バックアップから戻したもの）が、
**設定されていないドメインで認証を通る**。

### アカウントの存在は `auth_token_hash` で決まる

`envelope.json` ではない。署名フローで作られたアカウントは envelope を
持たないので、そちらを見ると 404 になる。M6c の掃除と同じ規則で、
**この 2 つは一致していなければならない**。

### 気づいた点

- 未知フィールドは**拒否せず無視**する（Go の `encoding/json` と同じ）。
  新しいビルド用の config が**切り戻しで死なない**ため
- `anchor_token` 検査に「警告して続行」は無い。
  認証なしの anchor 書き込みは、誰でも未取得の名前を主張でき、
  他人の名前を release して DNS ごと奪える
- `provision_domain()` は Go だと map を range するので**実行ごとに答えが変わる**。
  ソート順にした（SPEC §11.5）。開放ドメイン 2 つは元々設定ミスだが、
  **せめて毎回同じ壊れ方**をする

---

## 2026-07-28 — M6a（認証プリミティブ）完了 (`a6b8186`)

### 成果物

- `jmapserver/src/authtoken.rs` — `auth_token_hash` の生成・検証・4 種 base64 デコード
- `jmapserver/src/diddht.rs` — zbase32、did:dht 自己証明検証、署名対象文字列
- `jmapserver/src/devicekeys.rs` — デバイス鍵とセッショントークンの保存
- `devicekeys_interop` — **両実装が互いのファイルを読む**

テスト 225 件 green。

### ここは `data/` 互換の要

SPEC §5.2 の「ディスク形式は互換必須」が最も直接効く部分。検証した組み合わせ:

| | 意味 |
|---|---|
| Go が書いたデバイス鍵を Rust が読む | 移行方向 |
| Rust が発行したトークンを Go が受理 | 切り戻し方向 |
| **失効が両方向で効く** | 権限だけ消してセッションが残ると「失効済みデバイスが期限まで動く」 |
| 署名判定（正常 / 別データ署名 / 期限切れ / 未来 / 未登録デバイス） | 判断が一致するか |
| did:dht vouch（自分 / 別 ID / 非 did:dht） | anchor 無しでも動く経路 |

### 落としやすい挙動を明示テストで固定

`ListDeviceKeys` は `devices/` が無くても **`[]` を返す**（`null` ではない）。
Go のコメントに「`null.length` で Devices モーダルが本番で真っ白になった」と
書かれている類。テストで `to_string(...) == "[]"` まで確認した —
**移植で黙って落ちる典型**だから。

### また自分のテストが間違っていた

zbase32 のデコードで「要求より短い/長い」は両方失敗すると思っていたが、
Go は**要求バイト数を満たしたら残り入力を無視する**ので、少なく要求すると
切り詰めて成功する。did:dht は常に 32 バイトなので実害は無いが、
**契約を推測ではなく明記**するようテストを書き直した。

### 気づいた点

- `decode_auth_token` は base64 を 4 種類（標準/パディング無し/URL-safe/両方）
  順に試す。クライアントがまちまちで、**デコード失敗と誤トークンは区別できない**から
- 署名は**標準 base64**、デバイス鍵は**base64url**。混ざっている。
  URL-safe を受け付けないことをテストで固定した

### 次

M6 の残り: config パース → 認証層 → axum ルータ → 起動シーケンス。

---

## 2026-07-28 — M5f（OpenPGP）完了 = **M5 完了** (`e429dc1`)

### 成果物

- `crates/jmapsmtp/src/pgp.rs` — 鍵パース（armor / binary）、インライン暗号化、
  Autocrypt ピア鍵の保存
- `pgp_interop` — **交差復号**を両方向

テスト 211 件 green。**M5（MIME + SMTP + 暗号）が全部終わった。**

### 検証は交差復号しかない

セッション鍵が毎回変わるのでバイト比較は原理的に不可能。
「一方が封じたものを他方が開けるか」だけが意味を持つ。

| 方向 | 意味 |
|---|---|
| Go が Rust の暗号文を開く | **移行方向**。Rust 稼働中に保存したメールが切り戻し後も読める |
| Rust が Go の暗号文を開く | **既存データ**。ディスク上のメールは全部 Go が封じたもの |

corpus: 平文 / 非 ASCII / multipart / 空 / **全 256 バイト値** / 200KB。

後者の Rust 側復号は**テスト専用**（リレーは秘密鍵を持たず復号しない）。
それでも価値がある — **鍵パースとパケット形状が噛み合っていることの証明**になる。

### rand のバージョンが 3 つ

- workspace: `rand` 0.9
- `rsa` 0.9: `rand_core` 0.6
- `rpgp` 0.16: `rand` 0.8

`RngCore` が別トレイトなので混ぜるとコンパイルエラーになる。
エイリアス依存 (`pgp_rand = { package = "rand", version = "0.8" }`) で解決。
**RNG は「黙って弱くなる」より「コンパイルが通らない」ほうが正しい失敗**。

### またテスト側のバグ

Autocrypt の**非 armor 形式（生パケット）は UTF-8 ではない**のに
JSON の文字列フィールドに載せていた。Go 側でパケットが壊れ、
「Go が鍵をパースできない」ように見えた。base64 で運ぶよう修正。

**エラーメッセージが指す先が原因とは限らない** — 3 回目の教訓。

### 設計判断: `store_peer_key` は書く前にパースする

後で読めないファイルは、ファイルが無いより悪い。
「ピアの鍵がある」ように見えて、そのピア宛の送信が全部平文になる。

### M5 全体の総括

| | 検証方法 |
|---|---|
| MIME | 17 件の corpus を Go とバイト比較 |
| DKIM | Go の検証器が Rust の署名を受理 |
| Autocrypt | 決定的 4 関数をバイト比較 |
| SMTP 受信 | Go の `net/smtp` が配送できる |
| SMTP 送信 | 本物の go-smtp が受理 |
| PGP | 交差復号を両方向 |

**すべて「Go 実装を実際に走らせて」検証している。** 仕様書との突き合わせは 1 つも無い。

### 残り

**M6**: config + 起動シーケンス + axum ルータ + 認証 + 全エンドポイント + anchor。
これが終わると `just difftest` が oracle 対 Rust で意味を持つ。

---

## 2026-07-28 — M5e（SMTP 送信クライアント）完了 (`dc5c71b`)

### 成果物

- `crates/jmapsmtp/src/smtp_out.rs` — 送信クライアント（MX ルーティング込み）
- `smtp_interop` に**逆方向**を追加 — **本物の go-smtp サーバが受け取る**

テスト 194 件 green。

### lettre を使わず自前にした理由

**RCPT 拒否でも送信を続ける**挙動。lettre は提供していない。
「1 宛先が bounce したからメッセージ全体を失敗させる」のは複数宛先送信では誤り。
Go の挙動が正しく、それを再現するには自前が要る。

（結果として lettre 依存も外れた。`rustls` feature の綱渡りも消えた）

### 一発目は 4 件とも落ちた: 末尾に空行が増える

クライアントが本文の後に無条件で `\r\n.\r\n` を書いていた。
Go の `textproto.DotWriter` は**本文が既に改行で終わっていれば CRLF を足さない**。
つまり**受信側が保存する全メッセージに空行が 1 行増えていた**。

条件付きにして解決。理由を行に書いた。

**相互運用テストが無ければ絶対に気づかなかった** — 自前サーバ相手だと
両側が同じ勘違いをしていて通ってしまう。だから
`the_rust_client_and_server_agree_with_each_other` は
「単体では何も証明しない、両側がそろって流されるのを捕まえるためにある」
とコメントに書いた。

### コードに理由を書いた継承事項 2 つ

1. **STARTTLS は日和見・無認証**。ハンドシェイク失敗は平文で続行。
   ポート 25 では「無認証 TLS」の対案は「認証 TLS」ではなく「配送しない」
2. **最優先 MX しか試さない**。次点へのフォールバック無し。
   失敗させて上位のキューに再試行させる

### 気づいた点

- `check()` は**返答のクラス**で判定する（250 も 251 も RCPT 成功）。
  コード完全一致にすると server ごとの差で落ちる
- MX 名の末尾ドットは DNS では絶対名の印だが接続文字列では邪魔。
  剥がすのを単体テストで固定した

### 残り

**PGP 暗号化本体**（rpgp 統合）、**M6**（config + 起動シーケンス + axum ルータ）。

---

## 2026-07-28 — M5d（SMTP 受信サーバ）完了 (`a4f49b5`)

### 成果物

- `crates/jmapsmtp/src/smtp_in.rs` — 自前 ESMTP サーバ
- `smtp_interop` — **Go の `net/smtp`（リレー自身が送信に使う client）で叩く**

テスト 181 件 green。

### 検証の形: 「本物のクライアントが配送できるか」

受信サーバの本質はこれ。プロトコルの細部を仕様書と突き合わせるより、
**リレー自身が送信に使っている `net/smtp` を向けて配送させる**ほうが強い。
答えられない挙動があれば「配送失敗」として出る。

corpus: 通常配送 / 未知の宛先 / 宛先ゼロ / 大文字小文字 / dot-stuffing /
**250KB 本文**（バッファ境界越え）/ 1 接続 2 通 / 広告する拡張の一覧。

変異テストで検出力確認: dot-unstuffing を消す → 該当 1 件 FAILED、
`SMTPUTF8` を落とす → 拡張一覧 1 件 FAILED。

### バグに見えて仕様な挙動 2 つ

1. **未知の宛先を 250 で受けて黙って捨てる**
2. **宛先ゼロの DATA も 250 で受けて捨てる**

RCPT で拒否すると、**ポート 25 に到達できる誰にでもアドレスの存在を教える
オラクル**になる。Go の挙動をそのまま維持し、理由をコードに書いた。

### AUTH は広告しない

Go 側の session には `AuthPlain`（何でも通す）があるが、
**go-smtp 0.24 の auth インターフェースを実装していない**ので
ワイヤ上には何も出ない。public MX で AUTH を広告するのは
「どこにも届かない資格情報を送らせる」だけなので、
502 を返し拡張一覧にも出さない。相互運用テストで**不在を固定**した。

### STARTTLS は広告するが 454

`handle` は transport ジェネリックにしてあるので、
呼び出し側が TLS 側で再入すればいい。その配線は起動シーケンス（M6）と一緒。
**stall させるより拒否**するほうが日和見送信者には親切。

### 詰まった点: 10 分溶かしたデッドロック

各テストが「サーバをタスクで動かしつつ Go ヘルパを**同期的に**待つ」形。
`#[tokio::test]` の既定は current_thread なので、
**待っている間サーバタスクが 1 度も動かない**。

`flavor = "multi_thread"` に変更。理由を呼び出し箇所にコメントで残した。
（タイムアウトが 10 分だったので気づくのが遅れた。以降は短いタイムアウトで叩く）

### 残り

**SMTP 送信クライアント**、**PGP 暗号化本体**（rpgp 統合）、**M6**（起動シーケンス）。

---

## 2026-07-28 — M5c（Autocrypt / PGP-MIME）完了 (`c4f08ca`)

### 成果物

- `crates/jmapsmtp/src/autocrypt.rs` — ヘッダ注入/解析、RFC 3156 ラップ
- `autocrypt_interop` — 決定的な 4 関数を Go とバイト単位で比較

テスト 165 件 green。

### 見つけた: **リモート DoS**（M4 の Mailbox/set より重い）

`pgpMIMEWrapInline` が BEGIN / END マーカーを**それぞれ独立に**検索し、
「両方見つかった」しか確認しないまま `body[start : end+len(endMarker)]` する。
本文で **END が BEGIN より前**にあると slice が逆走して panic:

```
panic: runtime error: slice bounds out of range [32:25]
```

到達経路が悪い:

1. `sendEmail` は raw に BEGIN があれば必ず `pgpMIMEWrapInline` を呼ぶ
2. `sendEmail` は `OnSubmitEmail` 内の **裸の `go func()`** から呼ばれる
3. **回復されない goroutine の panic は Go プロセス全体を終了させる**

本文は認証済み送信者が自由に決める値。**メール 1 通でリレー上の全アカウントが停止**。

Rust は END を **BEGIN 以降から**探し、完全なブロックが無ければ
`None`（ラップせずそのまま送る）。SPEC §11.11。

**自分の実装も同じバグを持っていた** — Go を写したので当然で、
corpus に「逆順マーカー」を入れて初めて両方で露見した。

### 宣言済み差異の使い方が一段深まった

Go 側ヘルパに `recover()` を入れて panic を**観測可能な結果**にした。
テストは「Go は panic する / Rust は None を返す」の両方を要求する。

- Go が直れば → テストが「もう panic しない」と教える
- Rust が退行すれば → テストが落ちる

本物のリレーには `recover()` は無い。ヘルパにあるのは**比較のためだけ**で、
そのことをコメントに明記した。

### Go ヘルパは verbatim コピー

`autocrypt.go` の関数は `package main` の非公開なのでリンクできない。
verbatim コピーを置いたが、**この比較自体がコピーの鮮度を保証する** —
go-jmapsmtp 側が変われば赤くなる。信用ではなく検査。

### 気づいた点

- ヘッダ注入は「本文に `\nChat-Version:` があるだけ」でも既存扱いになる
  （Go は `bytes.Contains` を raw 全体にかけている）。重複注入よりマシなので踏襲
- PGP/MIME の boundary は**暗号文の sha1** なので出力が完全に決定的。
  だから相互運用テストがバイト比較できる
- 実際の OpenPGP 暗号化はセッション鍵がランダムなので、この方法では比較不能。
  交差復号テストが必要（未着手）

### 残り

**SMTP 受信サーバ / 送信クライアント**、**PGP 暗号化本体**（rpgp 統合）、
**config + main の起動シーケンス**（M6）。

---

## 2026-07-28 — M5b（DKIM）完了 (`36ebcd1`)

### 成果物

- `crates/jmapsmtp/src/dkim.rs` — 鍵の load-or-create、DNS レコード、署名
- `jmapsmtp` crate を lib + 薄い bin に分割（統合テストから触れるように）
- `dkim_interop` — **Go の検証器が Rust の署名を受理するか**

テスト 147 件 green。

### 検証の設計: 片方向だけでいい

**Go が Rust の署名を検証できること**が本質。Go の検証器はメッセージから
ヘッダ正準化・本文正準化の両方を再計算するので、受理された時点で
正準化・署名ヘッダ集合・鍵エンコードが全部一致していることになる。

逆方向（Rust が Go の署名を検証）は**テストしない**。
このリレーは署名するだけで検証しないので、**存在しないコードのテスト**になる。

### 2 回間違えた。どちらも単体では説得力があった

1. **grep するファイルを間違えて**「go-msgauth は `t=` を出さない」と誤認し、
   差異として SPEC に書きかけた。実際は両方出す
2. タグ抽出が `bh=` の中の `h=` にマッチしていたので、
   **ヘッダリストと base64 本文ハッシュを比較していた** —
   これが 1 の誤りを裏付けているように見えた

Go が実際に署名したヘッダを 1 回ダンプして両方まとめて解決。
**推測の連鎖より実測 1 回**。

### 残った本物の差異（最初に疑ったのと逆方向）

`h=` の順序:

- **Go**: 指定順 (`From:To:Cc:Subject:Date:Message-Id:Content-Type`)
- **Rust (mail-auth)**: **メッセージ中の出現順の逆** + 不在ヘッダを末尾
  (`Content-Type:Message-Id:Date:Subject:To:From:Cc`)

mail-auth のほうが RFC 6376 §5.4.2 推奨の bottom-up 方式
（転送中に前置されたヘッダが署名済みヘッダを押しのけられない）。
どちらも正当で、各実装は自分の `h=` と整合。**Go の検証が通ることで実証済み**。

合わせるには mail-auth の署名器を使わないしかないので、
SPEC §11.10 に記録し、テストは**ヘッダの集合**を比較する。
`bh=` / `d=` / `s=` / `a=` / `c=` / `v=` は完全一致。

### 気づいた点

- `.rev()` で入力を反転しても `h=` は変わらなかった。
  mail-auth の順序は**入力順ではなくメッセージのヘッダ順**に依存していた。
  一度試して初めてわかった
- `rsa` 0.9 は `rand_core` 0.6 を要求し、workspace の `rand` 0.9 と非互換。
  `rsa::rand_core::OsRng` を使う
- 破損した `key.pem` は Go だと**黙って新しい鍵を作る**（DNS レコードが取り残される）。
  Rust は `create_new` が既存ファイルで失敗するのでエラーになる。テストで固定

### 次

**M5c: SMTP。** 受信サーバ（自前 ESMTP + STARTTLS）と送信（MX 引き / relay_host）。
Autocrypt / PGP も残っている。

---

## 2026-07-28 — M5a（MIME）完了 (`45a3e99`)

### 成果物

`crates/jmapserver/src/mime.rs` — `ParseMIMEEmail` / `extractMIMEText` /
`ExtractAttachments` / `MessageBody` / `BuildRFC5322` / `BuildEnvelope`。
M4 でスタブにしていた `Email/import` と `Email/parse` も埋めた。

テスト 134 件 green。

### MIME crate に任せず手書きにした

Go は `net/mail` と `mime/multipart` に依存していて、**その特有の判断こそが
保存済みメッセージの中身**:

- Content-Type の無いパートは text/plain と見なさず**スキップ**
  （RFC 2045 の既定とは違う）
- アドレスリストは 1 件でも壊れていれば**ヘッダごと破棄**
- `multipart/encrypted` は text/plain フォールバックを**意図的に無視**
  （PGP メッセージの平文がサーバに残らないように）

汎用パーサは「MIME 一般」に強く「この MIME」に弱い。相互運用テストが判定者。

### パースは 17 件の corpus が一発で一致

組み立て側だけ差が出て、それが有益だった: `mail.Address.String()` は
**印字可能 ASCII の表示名を無条件で引用符で囲む** — `"Alice" <a@x>` であって
`Alice <a@x>` ではない。マルチバイトが入ると RFC 2047 encoded word に切り替わる。
どちらも RFC 5322 を読んでも出てこない。

### corpus が自分自身の flakiness を捕まえた

`Email.headers`（Chat-Group-Id など）は Go がヘッダマップを range して作るので
**実行ごとに順序が変わり**、その順序が**保存されるメッセージ JSON にそのまま入る**。

**1 回目は偶然通り、2 回目で落ちた。** 比較時にソートし、Rust 側は決定的に。
SPEC §11.5 に追記。3 回連続で安定を確認。

### 気づいた点

- 時刻と生成 Message-ID は**注入**にした（Go は直接 clock/RNG を読む）。
  でないと組み立て経路が比較不能
- HTML→Markdown は素通しのまま。§8-G の判断は
  **Go の変換器と実際に差分を取ってから**でないと決められない

---

## 2026-07-28 — M4（メソッド層）完了 (`3fd59b4`)

### 成果物

- `methods/` — RFC 8621 の 26 メソッド全部
- `dispatch.rs` / `refs.rs`（result reference）/ `server.rs`（Session・batch・auth・encode）
- `dispatch_interop` — **Go の `Store.Dispatch` と 47 呼び出しを突き合わせるテスト**

テスト 105 件 green。

### スコープを 1 つ落とした（正直に記録）

PLAN の M4 完了条件は「`just difftest` が oracle 対 Rust で green」だった。
これには **jmapsmtp バイナリ全体**（config・handler・auth・全ルート）が要る。
つまり実質 M4+M6 で、M4 単独では達成できない。

代わりに **dispatch レベルの相互運用テスト**を作った。Go と Rust が同じ store を
seed し、同じメソッド呼び出しスクリプトを**それぞれの Dispatch に通して**、
返る JSON を比較する。HTTP 層（ルーティング・CORS・SSE）は未検証で、
バイナリができる M6 に回す。**PLAN の完了条件も実態に合わせて書き換えた。**

axum のルータも今回は書いていない。アプリ層が無いと 1 行も実行できないため。

### 見つけた Go の実バグ: `Mailbox/set` が create 同時の update を捨てる

`mbByID` がスライスの backing array への**ポインタ**を持ち、create ループの
`append` で再確保が起きる。update ループは**捨てられた配列**に名前を書き、
最後の再構築は新しい配列を読む。Go 版で実測:

```
update のみ                → mbx-inbox=Renamed   （正常）
create + update 同時       → mbx-inbox=Inbox     （消失）
作成直後のものを update    → mbx-inbox=Inbox     （消失）
```

**どちらの消失ケースもレスポンスは `updated` に入れて成功を報告する。**
「1 つ作りつつ 1 つ改名」は正当な JMAP で一括整理では自然に出るので、
サイレントなデータ損失。移植しない（SPEC §11.7）。

### 宣言済み差異の機構

`dispatch_interop` のスクリプト各行に `divergence: Option<&str>` を持たせた。
宣言された差異は**一致と同じ厳密さで検査**する — **差が出なくなったら失敗**。

意図はこれ: 修正がリファクタで失われたとき、黙って Go のバグ挙動に戻るほうが、
元のバグより悪い。SPEC §11.9 に書いた機構を先に dispatch 層で実装した形。

### 順序の非決定性がさらに 2 つ出た

1. **`*/changes` の集合フィールド**（created/updated/destroyed/removed）は
   Go がマップを range して作るので**Go 対 Go でも順序が違う**。
   `queryChanges` は `index` までその反復順で振っている
2. **`Store::all` の同値タイ**: `sort.Slice` は**不安定ソート**で、
   入力もマップ順。`Email/copy` は `receivedAt` を引き継ぐので必ずタイになる

1 は比較時にソートして正規化。2 は**スクリプトの並べ替え**で対処した
（`Email/copy` を最後に置き、以降クエリしない）。正規化で潰すと
「順序が本当に壊れた」場合まで隠れるので、観測させないほうを選んだ。

### 詰まった点

- `Handler::authenticate` をトレイトのデフォルトメソッドにしたら、
  Go の「AuthFunc が**在るかどうか**で分岐が変わる」を表現できなかった
  （Rust ではトレイトメソッドが override されたか問い合わせられない）。
  Go の `Config.AuthFunc` と同じく**値**（`Option<AuthFn>`）に戻した
- ヒアドキュメントの `cd` が失敗して別ディレクトリに書きかけた。以降は絶対パス

### 気づいた点

- `identityState` は Go でも**永続化されていない**（`persistedState` に無い）。
  identities は残るのに state は再起動で 0 に戻る。そのまま移植
- `Email/copy` / `Email/import` / `EmailSubmission/set` は `oldState` も
  `newState` も**書き込み後**に読むので常に同値。実害が小さいので記録のみ（§11.8）
- `Email/import` と `Email/parse` は `ParseMIMEEmail` 待ちで空スタブ

### 次

**M5: MIME + SMTP。** 最大フェーズ。`email.go` の残り半分（892 行中の MIME 部）と、
自前 ESMTP サーバ、DKIM、Autocrypt/PGP。

---

## 2026-07-28 — M3 完了 (`fbd8748`)

### 成果物

- `crates/jmap-types` — Email / Mailbox / Address / Identity / Thread / Envelope / `JmapTime` / **`go_json`**
- `crates/jmapserver/src/store.rs` — Store 全体
- 相互運用ヘルパ `xtask/interop/store/`

テスト 82 件 green。うち `data/` 双方向相互運用 4 件が M3 の受け入れ基準。

### 一番の収穫: **Go の `encoding/json` は `<` `>` `&` を HTML エスケープする**

`\u003c` / `\u003e` / `\u0026`（加えて U+2028/2029）。serde_json は 5 つとも生で出す。

**メールでは致命的。** `inReplyTo` / `references` は `<id@host>` をそのまま保持するし、
件名の `&` も日常的。気づかなければ:

- 既存メッセージファイルに触れた瞬間に全部書き換わる
- 差分ハーネスが数百件の偽の差分で埋まり、本物の差分が埋もれる

→ `jmap_types::go_json` を作り、**ディスクに書く経路すべて**を通した。

**これはソースを読んでも仕様を推測しても出てこなかった。** Go 実装と実際に
突き合わせる相互運用テストだけが見つけた。

### そしてその修正自体が 2 回間違っていた

1. エスケープ表が `'<' => "<"` と**各文字を自分自身に写していた**（完全な no-op）
2. 単体テストの期待値も未エスケープ形だったので、**何も検証せずに green**

自己完結したウソ。Go 実装との比較だけが破った。M1 の `--self-test` と同じ教訓を、
別の場所でもう一度踏んだ形。

（余談: 修正の適用も 2 回失敗した。ファイル中に**実在する U+2028 文字**を
Python の `splitlines()` が改行として扱い、行が分割されて残骸が出た。
バイト単位で `\n` だけで分割して解決）

### `delta.json` は実測しないと絶対に書けなかった

`changeRecord` に **JSON タグが無い**ので、キーが `Added` / `Updated` / `Removed` と
**大文字始まり**。nil スライスは `null`（`[]` でも省略でもない）。

```json
{"state":2,"changes":{"1":{"Added":["msg-a"],"Updated":null,"Removed":null}},...}
```

推測で `added` と書いていたら、Go 版は**エラーも出さず空として読む**。
サイレントな状態消失になっていた。

### 時刻は文字列のまま保持することにした

`JmapTime` は**到着した文字列をそのまま**持つ。パースして再フォーマットすると:

- `+09:00` → `Z`、`.12` → `.120000000` に化ける
- Store は「1 フィールド触って全体を書き戻す」ので、**触るたびに全ファイルが書き換わる**

SMTP 受信経路は `time.Now()` を `.UTC()` なしで使っているので、
**ローカルオフセット付きの時刻が実際にディスクにある**。順序比較のときだけパースする。

### Go のマップ反復順に由来する非決定性

Go が `map` を range する箇所の出力順はランダム:

- `Purge` の `Removed` 配列
- `SyncMailboxes` の `Created` / `Destroyed` 配列
- 同一 Message-ID 複数時の `resolveThreadID` の勝者

**Go 対 Go でも一致しない**ので「合わせる」対象が無い。`BTreeMap`/`BTreeSet` で
決定的にした（SPEC §11.5）。

ついでに**差分ハーネスの潜在バグを 1 つ塞いだ**: `delta.json` を素の文字列比較して
いたので、シナリオが一度に 2 つ以上のメールボックスを作った瞬間に
Go 対 Go でも落ちるところだった。比較前に配列をソートする正規化を入れた。

### あえて直さなかったもの: `safeFilename` の衝突

`/ \\ : * ? " < > |` を全部 `-` に置換し 200 文字で切る **多対一の写像**。
`a/b` と `a:b` が同じファイルになる。長い AP URL は切り詰めでも衝突する。

**直さない。** ファイル名そのものが on-disk フォーマット（§5.2 の `data/` 互換要件）。
別方式にすると既存ファイルが旧名で取り残され、新規書き込みだけ新名になり、
**同じメッセージが 2 つの内容で読める**状態になる。テストで明示的に固定した。

### 自分のテストが間違っていた件

`references_are_walked_when_in_reply_to_misses` が落ちた。原因は私の想定違いで、
Go は新スレッド ID を作るとき Message-ID を**トリムしない** —
索引側だけ `<>` を剥がす非対称な作りだった。正しい期待値は `thr-<parent@x>`。
コメントに非対称性を明記して残した。

### 気づいた点

- `Email` の全フィールドが `omitempty` なので `Email{}` は `{}` になる。
  `skip_serializing_if` を 1 つ落とすだけで壊れるため、専用テストで固定
- `preserve_order` を M0 で入れていたが**外した**。Go はマップキーをソートするので、
  挿入順保持は不一致になる
- Store のフックは型だけ定義して未配線（呼ぶのは M4 の dispatch）

### 次

**M4: JMAP HTTP サーバ。** Session / `/jmap/api/` / SSE / blob / CORS / 26 メソッド。
`just difftest`（oracle 対 Rust）が初めて意味を持つフェーズ。
SPEC §11.9 の divergence 宣言機構もここで入れる。

---

## 2026-07-28 — M2 完了 (`d218fb2`)

### 方針変更: Go 版のバグは移植しない

ユーザ指示。**Go 版が常に正しいわけではない**ので、明らかにおかしい箇所は直す。
ただし差異は `SPEC.md §11` に必ず記録する — 記録のない差異は移植ミスと区別がつかない。

§11 を「唯一の例外」から「意図的な差異の一覧」に格上げし、
各項目に **Go の挙動 / 変更後 / 理由** を書く形式に変えた。

### 最初に呼び出し側を読んだら、モジュールの性格が変わった

`cryptenv.` の全使用箇所を grep したところ、**サーバは
`Unseal` / `Rewrap` / `VerifyAuth` / `NewEnvelope` を一度も呼んでいない**。
使うのは `FromBytes` と `Bytes()` だけ。パスワードはサーバに届かないので、
サーバ側では原理的に復号できない。

つまりエンベロープはサーバにとって**完全に不透明**で、
`FromBytes` だけが唯一「実際に動く」関数。ここが甘いと直接被害が出る。

### 見つけたバグ: 空の JSON でアカウントを永久に潰せる

`FromBytes` は `json.Unmarshal` するだけで**何も検証しない**。Go 版で実験:

```
{}                → err=<nil>  env=&{Version:0 Salt:[] KDF:{0 0 0} ...}
null              → err=<nil>  env=&{Version:0 ...}
{"v":1}           → err=<nil>  env=&{Version:1 ...}
```

これで何が起きるか:

1. `POST /auth/signup?token=X` に `{}` を送る
2. **一度きりの setup token が消費され削除される**
3. ゼロ値エンベロープが `envelope.json` に書かれ、204 が返る
4. 以降 signup は「already initialized」で 409
5. どんなパスワードでも復号できない → **アカウントが永久に使用不能**

**未認証**で実行できる。setup token は 16 バイトなので総当たりは非現実的だが、
トークンは起動時にログへ平文で出るし、正規ユーザが壊れた JSON を送るだけでも同じ結果。

→ `from_bytes` で**復号が原理的に不可能な値だけ**を拒否するようにした（§11.2）。
`t=1, m=8, p=1` や 8 バイト salt は通す。**検証はポリシーの押し付けではなく、
不可能値の排除に限る**という線引きをテストにも書いた
(`accepts_unusual_but_workable_parameters`)。

副次的に: `kdf.t=0` / `p=0` は **Go の argon2 が panic する**値。
サーバは Unseal しないので現状は踏まないが、潜在的な地雷ではある。

### もう 1 つ: `Rewrap` だけバージョン検査が無い

`Unseal` は `e.Version != currentVersion` を見るが `Rewrap` は見ない。
将来の v2 エンベロープが v1 の解釈で黙って rewrap される。
非対称にする理由が見当たらないので両方で検査（§11.3）。
サーバは Rewrap を呼ばないので HTTP レベルの差異なし。

### 相互運用テスト

`xtask/interop/cryptenv/main.go` を `just interop` で **oracle チェックアウト内に
コピーしてビルド**する。これで Go 版の**本物の** cryptenv パッケージにリンクされる
（再実装ではない）。Rust 側がサブプロセスとして駆動。

検証した方向:

| テスト | 意味 |
|---|---|
| `rust_opens_a_go_sealed_envelope` | **移行方向**。既存デプロイの `envelope.json` は全部 Go 製 |
| `go_opens_a_rust_sealed_envelope` | **切り戻し方向**。Rust 稼働中に作られたアカウントが失われないか |
| `go_rejects_the_wrong_password_...` | 誤パスワードが両側で失敗するか |
| `go_opens_a_rust_rewrapped_...` | rewrap 後も派生鍵が不変か |
| `interoperates_at_the_real_cost_parameters` | **p=4 は p=1 と別コードパス**。実パラメータで 1 回だけ確認 |

### 変異テストで検出力を確認（M1 の規律を継続）

`HKDF_INFO_AUTH` を `v1` → `v2` に改変して実行:

- 単体テスト 2 件 FAILED（`hkdf_info_strings_are_frozen`, `hkdf_derivation_...`）
- 相互運用テスト **3/5 FAILED**（`auth_token differs`）

**そしてこれが穴を 1 つ露出させた。** ヘルパのバイナリを消して実行すると
**「5 passed」と表示される**（0.00s で）。何も実行していないのに green。
M1 で潰したはずの「落ちないテスト」がここに再発していた。

→ `CRYPTENV_INTEROP=required` を導入。`just test` が設定するので、
**通常のワークフローではヘルパ欠落が明示的なエラーになる**。
素の `cargo test` は Go 未インストール環境のために skip のまま。

### 気づいた点

- `hkdf_derivation_matches_the_go_implementation` の期待値は
  **Go 側で実際に計算させた**もの（`master_secret = 0x42 × 32`）。
  自前実装の出力を貼ると自己満足のテストになる
- `go_opens_a_rust_rewrapped_envelope_with_unchanged_keys` は
  Go の gen → Rust rewrap → Go unseal なので、**Rust の派生ロジックは比較していない**
  （rewrap が master_secret を保存したかだけを見る）。派生は他のテストがカバー
- `Unsealed` に `Debug` を自前実装して `<redacted>` にした。
  panic メッセージに鍵が載るのを防ぐ

### 次

**M3: `jmap-types` + `Store`。** 699 行 + 型定義で、ここから規模が大きくなる。
`data/` の双方向互換が受け入れ基準。

---

## 2026-07-28 — M1 完了 (`5ef227c`)

### 成果物

- `xtask difftest` — 差分ハーネス（`xtask/src/difftest/` 6 ファイル）
- `SPEC.md` — ハーネスで検証**できない**事項の記録
- `xtask/fixtures/` — 決定性のために両サイドへ配る固定鍵

```
just difftest-selftest → 4 変異すべて検出
just difftest-oracle   → 46 ステップ差分なし
just difftest          → oracle 対 Rust（M4 以降で使う）
just difftest-filters  → 正規化フィルタ一覧を表示
```

### 判断 1: 「正規化するより、決定的に仕込む」

リレーは初回起動時に DKIM 鍵・自己署名 TLS 証明書・setup トークンを**ランダム生成**する。
放置すると 3 つとも両サイドで食い違い、それを潰すフィルタが必要になる。
だがそのフィルタは**鍵の扱いを間違えた本物のバグも同時に隠す**。

→ fixture 側で全部**事前に配る**ことにした。フィルタが 1 つ減るたびに
死角が 1 つ減る。

同じ理屈で `base_url`。**両サイドで同一の `base_url`**（`http://relay.difftest.invalid`）を
使い、`listen_addr` だけ変える。JMAP Session レスポンスは `base_url` を
`apiUrl`/`downloadUrl`/`uploadUrl` に echo する（server.go:311）ので、
ポートを変えると URL フィルタが要る → **間違った URL が素通りする**ようになる。

### 判断 2: 「落ちないハーネスは、無いより悪い」

`--both-oracle` が**一発目で green になった**。喜ぶところではなく、
**「何も比較していないハーネス」と区別がつかない**状態。

→ `--self-test` を追加（当初計画に無し）。oracle を意図的に壊したコピーと比較し、
比較軸ごとに 1 つずつ変異を用意して**全部検出されること**を要求する:

| 変異 | 壊す対象 | 検出数 |
|---|---|---:|
| `RelayLabel` | レスポンスボディ | 1 |
| `BaseUrl` | Session レスポンス | 3 |
| `ExtraDataFile` | `data/` ツリー | 1 |
| `BreakCredential` | 認証経路 | 78 |

**そしてこれが実際にバグを見つけた。** シナリオの WKD ステップが
`8xnqfyeqrbanrhqoq5b6ba6a1kzjxfyy` を「zbase32(sha1("alice"))」と称していたが、
Go 実装で実際に計算すると `kei1q4tipxxu1yj79k9kfukdhfy631xe`。
つまり**ハッシュ一致分岐をテストしているつもりで、不一致分岐をテストしていた**。
3 ステップ（一致 / 不一致 / localpart なし）に分割して修正。

### 判断 3: SPEC.md には「ハーネスで検証できないこと」だけ書く

当初計画では SPEC.md を全仕様の網羅文書にするつもりだったが、
ハーネスが動く以上、HTTP レスポンスの正解は **oracle が生成する transcript** であって
文章ではない。両方に書くと**必ず片方が腐る**。

→ SPEC.md の守備範囲を「リクエストを投げるだけでは観測できない事柄」に限定した:
起動シーケンスの順序、6 時間周期の purge、凍結された暗号定数、ディスク形式の**意味**、
SMTP プロトコル挙動、メッセージ変換パイプラインの段構成、外部サービス通信。

### ハーネスが早速ピン留めした Go 版の癖

- **CORS ヘッダがルートごとに違う**。`/jmap/api/` は `GET, POST, OPTIONS`、
  `WrapCORS` 経由の 404 は `GET, POST, PUT, OPTIONS`。
  汎用ミドルウェアで統一すると壊れる（PLAN.md §5.1 の警告が実物で確認できた）
- **`Email/set` の create は `messages/` に何も書かない**。`OnCreateEmail` が
  `PutPending` を呼ぶだけなので、submit されるまで永続化されない。
  `newState` も `"0"` のまま動かない

### 詰まった点

なし。強いて言えば `--both-oracle` が一発で通ったことが最大の落とし穴だった（判断 2）。

### 気づいた点（後で対処）

- `transcript.txt` が実質「Go 版の振る舞いの読める仕様書」になっている。
  M4 以降でシナリオを増やすほど価値が上がる。SPEC.md に書きたくなったら
  まずシナリオに足せないか考える
- シナリオは現状 46 ステップ。SMTP 配送（M5）、anchor 付き経路（M6）、
  PGP 鍵をアップロードした状態の WKD（M6/M7）が未カバー

### 次

**M2: cryptenv。** 最初の縦切りで、暗号スタック（argon2 / aes-gcm / hkdf / subtle）の
疎通確認を兼ねる。Go 版と `envelope.json` を相互に読ませる。

---

## 2026-07-27 — M0 完了 (`70874c6`)

### やったこと

- rustup 導入 → **rustc 1.97.1** (stable, default profile なので clippy/rustfmt 同梱)
- `~/.bashrc` に `. "$HOME/.cargo/env"` を追加（rustup の既定挙動を明示的に実施）
- `git init` → workspace 雛形 5 クレート + justfile + .gitignore + README + config.example.json
- `just oracle` で Go 版 oracle をビルドする仕組みを整備

### 検証結果（すべて green）

```
cargo build --workspace                              → OK
cargo build --workspace --no-default-features        → OK  (= go build -tags noanchor)
cargo clippy --workspace --all-targets -- -D warnings → 無警告
cargo fmt --all --check                              → clean
cargo test --workspace                               → OK (テストはまだ 0 件)
just oracle-check                                    → oracle 2 種ビルド + Go テスト全 pass
                                                       go-jmapserver の 39a4d0e からの drift なし
```

### 判断とその理由

**workspace を 5 クレートに分けた。** 1 クレートに全部入れても動くが、
`crates/jmapserver` から `jmapsmtp` 固有の型を参照しない制約（§8-F-2、将来の切り出し用）を
**コンパイラに強制させたい**ため。人間の規律に頼ると必ず漏れる。

**各 stub には「何を入れるか + 移植元の Go ファイル」だけ書いた。**
中身が空でも、ファイルを開けば担当範囲がわかる状態にしておく。

**`oracle/` は .gitignore。** リポジトリ外の 2 つの Go リポジトリからのビルド生成物であって、
このリポジトリの成果物ではない。代わりに `justfile` に再現手順を持たせた。
`just oracle-check` は go-jmapserver の drift（`git log 39a4d0e..HEAD`）も報告する（§8-A' 対策）。

**clippy は `correctness` / `suspicious` のみ deny。**
移植は Go の制御フローを意図的になぞる（§8-D）ので、`pedantic` や `style` の
「もっと Rust らしく書け」という指摘は移植中はノイズになる。
バグを示すカテゴリだけ硬いエラーにした。

### 詰まった点

**lettre の feature 指定でビルド失敗。** `default-features = false` +
`["smtp-transport", "tokio1-rustls", "builder"]` だけでは
「crypto provider が無い」「cert verifier が無い」で `compile_error!`。

→ `ring` + **`rustls-native-certs`** を追加。`webpki-roots` ではなく native を選んだのは、
Go の `crypto/tls` が OS の root store を使うため。送信 STARTTLS の信頼判断を Go 版と揃える。

### 気づいた点（後で対処）

- **hickory-resolver が 2 バージョン入っている**: 自前指定の `0.25.2` と、
  `mail-auth` が引く `0.26.0-alpha.1`。ビルドは通るがコンパイル時間の無駄。
  M5 で MX 解決を書くとき、`mail-auth` 内蔵の Resolver を使えば自前の hickory 依存を
  外せるかもしれない。**M5 で判断する**
- 依存解決の実績バージョン: `pgp 0.16.0` / `mail-parser 0.11.5` / `mail-auth 0.7.5` /
  `htmd 0.2.2` / `axum 0.8` / `rustc 1.97.1`

### 次

**M1: 差分ハーネス。** `xtask difftest --both-oracle`（oracle 対 oracle）が green に
なるまでが M1 の本体。正規化フィルタの設計がすべて。

---

## 2026-07-27 — 決定事項の確定、M0 着手

### ユーザ確定事項

| # | 論点 | 決定 |
|---|---|---|
| §8-F-1 | `data/` 互換 | **必須**。既存デプロイをバイナリ差し替えで移行する |
| §8-F-2 | go-jmapserver の扱い | **本リポジトリの workspace crate として取り込む** |
| §8-F-3 | ドキュメントの言語 | `PLAN`/`SPEC`/`JOURNAL` は日本語、`ARC`/`README`/`MIGRATION` は**英語** |
| §8-E | デバッグ用 `.eml` ダンプ | **`debug_dump_eml` フラグを追加し既定 off**。Go 版と意図的に挙動を変える唯一の箇所 |
| §8-A | go-jmapserver のソース | `~/go-jmapserver` (`39a4d0e`) 提供により**解決** |

§8-F-2 の帰結として、`crates/jmapserver` からは `jmapsmtp` 固有の型を一切参照しない
制約を M3 以降ずっと守る。将来 `go-jmapap`（AP リレー）を Rust 化する際に
独立リポジトリへ切り出せるようにするため。

---

## 2026-07-27 — 初期調査と PLAN.md 作成

### 最初に踏んだ罠: go-jmapserver が無くてビルドできなかった

`go-jmapsmtp/go.mod` に `replace github.com/yno9/go-jmapserver => /Users/n/go-jmapserver`
があり、この作業コピーがマシンに無かった。GitHub の公開版 (`6ad07a1`) を clone して
差し替えたが、以下が未定義でビルド失敗:

```
undefined: jmapserver.Attachment / CheckSessionToken / IssueSessionToken
undefined: jmapserver.ListDeviceKeys / WriteDeviceKey / RemoveDeviceKey
undefined: jmapserver.VerifyDeviceSession / VerifyDidDhtVouchLocal / ExtractAttachments
undefined: anchor.VouchDevice / anchor.DeviceVouchProof
```

公開版には `devicekeys.go` / `diddht.go` が丸ごと無かった（未 push）。

この時点では「動く参照実装が使えない」前提で計画を立てた。仕様はソースとテストから
読み取るしかなく、`devices_test.go` から署名フォーマットだけは復元できたが、
**デバイス鍵とセッショントークンのディスク形式が不明**で、`data/` 互換を諦めるか
どうかが最大の未決事項だった。

### 解決: `~/go-jmapserver` (`39a4d0e`) の提供

ユーザが最新版を clone してくれて解決。検証:

```bash
cp -r ~/go-jmapsmtp <scratch>/gobuild
sed -i 's|=> /Users/n/go-jmapserver|=> /home/ubuntu/go-jmapserver|' <scratch>/gobuild/go.mod
cd <scratch>/gobuild
go build ./...                # → OK
go build -tags noanchor ./... # → OK
go test ./...                 # → ok github.com/yno9/go-jmap-smtp
                              #   ok github.com/yno9/go-jmap-smtp/cryptenv
cd ~/go-jmapserver && go test ./...  # → ok jmapserver / ok jmapserver/anchor
```

**これが計画の性格を変えた。** 動く oracle があるので、差分テスト
(Go 版と Rust 版に同一入力 → 出力をバイト比較) を主軸にできる。
M1 の主眼を「SPEC.md を書く」から「**差分ハーネスを作る**」に変更した。

M1 の完了条件をあえて **Go 版 対 Go 版（同一バイナリ 2 プロセス）で green** に
したのは、正規化フィルタ（乱数 ID・タイムスタンプ・PGP セッション鍵・Go の map
反復順を潰す）が正しいことを、Rust を書く前に確かめるため。ここが甘いと
「Rust のバグ」と「フィルタの不備」を区別できなくなる。

### `devicekeys.go` から判明した形式

ヘッダコメントに明記されていた:

```
<acctDir>/devices/<deviceID>.json     {"id","label","created_at"}
<acctDir>/sessions/<tokenHash>.json   {"device_id","expires_at"}
```

- `deviceID` = `base64url(ed25519 公開鍵 32B)`（decode は RawURL → URL の順に試行）
- セッショントークン = `base64_std(random 32B)`。**保存されるのはハッシュのみ**
- `tokenHash` = `base64url_nopad(sha256(raw token))`
- 署名対象: `session:<did>:<devicePubKey>:<ts>` / `devkey:<did>:<devicePubKey>:<label>:<ts>`
- 時刻ずれ許容 300 秒、`did:dht:<zbase32(ed25519 pubkey)>` は自己証明的でネットワーク不要

→ `data/` 完全互換が達成可能になった。

**移植時に落としやすい挙動 2 点**（Go 側に「本番で踏んだ」とコメントがある）:

1. `ListDeviceKeys` は `devices/` が無くても**必ず非 nil の空配列**を返す。
   `null` を返すとクライアントの `devices.length` が例外を投げて Devices モーダルが
   真っ白になる
2. `RemoveDeviceKey` はデバイスファイルだけでなく、**そのデバイスに発行済みの
   セッションファイルも全部消す**。消さないと revoke 済みデバイスが
   トークン自然期限まで動き続ける

### 移植規模の確定

| | 非テスト | テスト |
|---|---:|---:|
| go-jmapsmtp | 3,758 | 860 |
| go-jmapserver（`cmd/` 除く） | 5,113 | 815 |
| go-jmap 型（必要サブセット） | 〜1,500 | — |
| **計** | **約 10,400** | **1,675（約 45 本）** |

最大ファイルは `email.go` (892行, MIME parse/build)、次いで `store.go` (699), `server.go` (562)。

### 環境

- Rust ツールチェイン **未インストール** → M0 で導入
- Go 1.22.4 あり（1.26.3 を自動 DL）
- crates.io 到達可（`index.crates.io` / `static.crates.io` とも 200）
- `~/jmapsmtp/` は空だった

### 新たに認識したリスク

- **§8-G HTML→Markdown 変換**: `email.go:641` が `JohannesKaufmann/html-to-markdown` を
  使用。Rust 側（`htmd`/`html2md`）は同じ HTML から違う出力を出す公算が大きく、
  差分テストが確実に赤くなる。影響は「HTML メール受信時の `textBody`」。
  M5 で実差分を見てから対応を決める（先に決め打ちしない）
- **§8-A' oracle の陳腐化**: `~/go-jmapserver` が移植中に更新されると差分が出るが
  Rust のバグではない。基準を `39a4d0e` に固定し、M8 で `git log 39a4d0e..HEAD` を確認
