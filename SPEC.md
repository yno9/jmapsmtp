...
- **この 2 行は Go 実装の記述であって、移植側の契約ではない。**
  移植側は到着時に保存する（**§11.17**）

### 送信

- **`MAIL FROM` に ESMTP パラメータを付ける**（`net/smtp` の `Client.Mail`）:
  相手が `8BITMIME` を広告していれば ` BODY=8BITMIME`、
  `SMTPUTF8` を広告していれば ` SMTPUTF8`。**この順序**。

  裸の `MAIL FROM` は等価ではない ――
  `BODY=8BITMIME` は「本文に 8bit オクテットが含まれうる」の宣言で、
  付けないと厳格なサーバは 7bit と仮定してよく、
  **UTF-8 の件名や本文が壊れるか拒否される**。

  `smtp_client_dialogue_interop` が、両クライアントを
  **同じ記録サーバ**に向けて発話を比較することで固定する。
  それまでの検査は「何が届いたか」を見ており、
  **何を言ったか**は誰も見ていなかった。
...
### 11.17 【重大】受信バッファが溢れると、250 を返したメールを捨てる

Go は受信メールを**到着時に保存しない**。
容量 256 のチャネル（`main.go` の `bufCh`）に積み、
**JMAP リクエストが来たとき**に `drainBuffer` で流し込む
（`Dispatch` の先頭で呼ばれる）。

JMAP リクエストの間に 257 通目が来ると、
`bufferEmail` は `default` 節に落ちてログを出し、**そのメールを捨てる**。

**SMTP の会話ではすでに `250` を返している。**
送信側 MTA にとって `250` は「受け取った、再送するな」の意味なので、
**このメールは誰にも届かず、誰も気づかない**。

oracle で実測（1 接続で 300 通、間に JMAP リクエストなし）:

```
配送:     300 通すべて 250
Email/query: 256 件      ← 44 通が消えている
```

同じ 300 通を、間に `Email/query` を挟んで 128 通ずつ配送すると **300 件**。
つまり原因は配送経路ではなくバッファである。

#### なぜ移植側で再現しないか

これは**沈黙するメール消失**であり、互換性の対象になりえない。
「Go と同じように壊れる」ことに運用上の価値が無い。

移植側は `Delivery::deliver` で**到着時にそのまま保存する**。
積む場所が無いので溢れる場所も無い。

- 到着から可視までの遅延も無くなる（Go は JMAP リクエストが引き金）
- 容量上限は `max_account_storage_mb` で**別途**見ている（§5）。
  こちらは拒否であって、受理してから捨てるのではない

#### 検証

2 つで 1 組。片方だけでは、もう片方が変わったときに黙って通る:

- `delivery::tests::more_than_256_messages_arriving_between_requests_are_all_stored`
  — 移植側が 300 通すべて保存する
- `inbound_buffer_interop` — **oracle が今も捨てること**を要求する。
  Go 側が直れば、この差異宣言が陳腐化したとして落ちる

> §5 の「容量 256 のチャネルに積む。溢れたらログを出して捨てる」は
> **Go 実装の記述**であり、移植側の契約ではない。

---

### 11.18 SMTP を bind するのは HTTP より先

Go は `go startSMTP(h, dataDir)` で**別 goroutine に投げて**から、
main goroutine で `http.ListenAndServe` する。
つまり **HTTP が応答を返し始めた時点で、ポート 25 はまだ bind されていない**ことがある。

実測（`inbound_buffer_interop` を書いている最中に踏んだ）:
oracle の HTTP が応答した直後に SMTP へ繋ぐと、
**3 回に 1 回程度 `Connection refused`**。
oracle のログでも `jmap listening` が `[smtp] listening` より先に出る。

bind に失敗した場合は goroutine の中で `log.Fatalf` するので
プロセスは死ぬが、**死ぬまでの間 JMAP には答え続ける**。

移植側は `main.rs` のステップ 14 で **SMTP を先に bind し、bind を await する**。
HTTP が応答するときには必ずポート 25 が上がっている。

#### なぜ揃えないか

**JMAP は受けるがメールは受けないリレー**は、
まだ起動していないリレーより悪い —
**監視から見ると健全に見える**から。
ヘルスチェックが「HTTP 200」を見ている限り、
Go ではこの窓が観測できて、移植側では観測できない。

`inbound_buffer_interop::connect_smtp` が oracle 側の待ちを持っており、
その待ちが必要であること自体がこの差異の証拠になっている。

---

### 11.19 JMAP ルートの認証は 3 形式（移植漏れ）

`go-jmapserver/server.go` の `extractCredentials` は、この順で受け付ける:

1. `Authorization: Bearer <email>:<password>`（コロン無しなら username 空）
2. `Authorization: Basic <base64(user:pass)>`
3. `?access_token=<email>:<password>`

対象は `s.auth` が包むルートのみ ――
`/.well-known/jmap` / `/jmap/api/` / `/jmap/eventsource/` /
`/jmap/push/subscribe` / `/jmap/push/unsubscribe` / blob。

**リレー自身のルート**（`/account/*` など）は `auth_env.go` の
`authenticate` で **Basic のみ**。2 つの表面で 2 つの契約であり、
混ぜるとリレー側の口が広がる。

移植側は当初 **Basic しか受けていなかった**。

#### なぜ本番で初めて出たか

**biset は 3 つのうち 2 つを使い、Basic を使わない**:

- `/jmap/api/` → `Bearer <email>:<token>`
- `/jmap/eventsource/` → `?access_token=`（`EventSource` はヘッダを付けられない）

移植側の interop テストは**全部 Basic** で認証していたので、全部通っていた。
実クライアントは**一切ログインできない**のに。

本番の caddy アクセスログで確定した:

```
4x POST /jmap/api/                    -> 401
2x GET  /jmap/eventsource/?access_token=… -> 401
1x GET  /relay-info                   -> 200
```

`server::tests` の `the_jmap_routes_take_a_credential_three_ways` ほかが
3 形式と**優先順位**（ヘッダがクエリに勝つ / Bearer が Basic に勝つ）を固定する。

---

### 11.20 パスから取り出す識別子はパーセントデコードされている（移植漏れ）

Go の `r.URL.Path` は **net/http が復号済み**の値を渡す。
`strings.TrimPrefix(r.URL.Path, "/contacts/")` は
`urn:uuid:…` を返す。

axum の `Uri::path()` は**生**なので、同じコードが
`urn%3Auuid%3A…` を返していた。

影響:

- `PUT /contacts/<uid>` ―― パスの uid と本文の uid が一致せず **400**。
  **ブラウザからの連絡先書き込みが全部失敗する**
- `GET /admin/accounts/<address>` ―― `@` が `%40` で来ると同様

移植側は該当箇所で `percent_decode` を通すようにした。

#### なぜテストで出なかったか

`contacts_interop` の uid が **`uid-1`** ―― エスケープの要る文字が 1 つも無い。
実クライアント（biset）の uid は `urn:uuid:…` で、ブラウザは `:` を `%3A` にする。

**「素の値だけ試す」は「Basic だけ試す」（§11.19）と同じ形の失敗。**

本番の caddy アクセスログで確定:
`PUT /contacts/urn%3Auuid%3A66f701df-… -> 400`。

`a_uid_that_needs_escaping_round_trips_on_both` が、
oracle がエスケープ付き uid を 204 で受けることと、
**未デコードの uid が本文と一致しないこと**の両方を要求する。

---

### 11.21 動的アカウントのストアにフックが付いていなかった（移植漏れ）

`Email/set create` と `EmailSubmission/set` は**アプリ側のフック**で実装される
（ストアは id も Message-ID も受信時刻も作れない）。
フックが無いストアは create に対して
`serverFail: Email/set create not configured` を返す。

Go は `makeStore` の中で `OnCreateEmail` を設置し、
**すべてのアカウントがそこを通る**（`addDynAccount` も静的も）。

移植側はフック設置が別ステップで、
**動的アカウントの経路が呼び忘れていた**。
設定に静的アカウントが 0 件のリレー（provisioning を使えば普通の形）では、
**誰も送信できない**。

#### 受信は通っていた

`Delivery::deliver` はストアのフックを使わないので、
**受信は動き、送信だけ死ぬ**という形になった。
「アカウントが使える」の半分だけを見ていたことになる。

`a_dynamic_account_can_create_an_email` が、
scan で復旧したアカウントに対して `Email/set create` が
id を返すことを要求する。

---

### 11.22 【重大】STARTTLS 後にバナーを再送していた（移植漏れ）

`220` のバナーは**接続**に対する応答であって、コマンドに対する応答ではない。
go-smtp は `greet()` を `Serve` から**一度だけ**呼び、
昇格後の `init()` は reader/writer を作り直すだけでバナーを出さない。

移植側は昇格後にセッションを再開する際、先頭のバナーごと再実行していた。
結果、**以降の応答が全部 1 つずれる**:

| クライアントが送る | 受け取る応答 |
|---|---|
| `EHLO` | `220` バナー |
| `MAIL FROM` | `EHLO` の 250 行 |
| `RCPT TO` | `MAIL` の応答 |
| `DATA` | **`RCPT` の応答** |

**TLS を使う MTA からのメールが全部拒否される。** そして実質すべての MTA が使う。

実際のバウンス（Postfix, mail.1n.ma）:

```
<y@biset.md>: Protocol error: host mail.biset.md said:
  250 2.0.0 I'll make sure <y@biset.md> gets this (in reply to DATA command)
```

#### なぜテストが通っていたか

`a_client_can_upgrade_the_session_with_starttls` は
**ハンドシェイクが起きること**しか見ていない。
昇格は成功していたので、ずっと green だった ――
**その後の会話を誰も見ていなかった**。

`smtp_dialogue_interop`（oracle と全応答を突き合わせる）も平文のみ。
oracle の fixture に証明書が無く、STARTTLS が広告されないため、
**昇格後の会話は比較対象に入っていなかった**。

`a_session_resumed_after_starttls_does_not_greet_again` が、
再開セッションの最初の応答が `220` **でない**ことと、
`DATA` が `354` で答えられることを固定する。

---

### 11.23 【重大】受信メールを inbox に入れていなかった（移植漏れ）

Go は受信時に 3 つを設定する（`main.go` の受信経路）:

```go
e.ID         = makeMessageID(rawMsgID, primary, now)
e.MailboxIDs = map[jmap.ID]bool{makeMailboxID(primary): true}
e.ReceivedAt = &now
```

移植側は **id と receivedAt だけ**を設定し、`mailboxIds` を設定していなかった。

`mailboxIds` の無いメッセージは**どのメールボックスにも属さない**。
ディスクには在り、`Email/get` は id で返すのに、
**メールボックスを列挙するクライアントには空に見える**。

biset には届いたメールが 1 通も表示されなかった。

#### なぜテストが通っていたか

`delivery/tests.rs` の受信テストは全部
「**保存されたか**」（`store.all().len()`、`get` が返すか）だけを見ていた。
**`mailbox_ids` を見るテストが 1 つも無かった。**

「届いた」と「見える」は別で、片方しか検査していなかった。

`a_delivered_message_is_filed_into_the_inbox` が
`mbx-<address>` に入ることを固定する。

#### 受信時の PGP 封緘（移植済み・2026-08-11）

Go は受信時に**受信者の公開鍵で本文を暗号化**する
（既に PGP なら `$e2e` を付けるだけ）。移植側にこの経路が無く、
`seal_stored_body` は送信済みコピー（`submit.rs`）からしか呼ばれて
いなかった。つまり**送ったメールは封緘され、受け取ったメールは
平文でディスクにあった**。

表示の問題ではなく機密性の差であり、だからこそどの比較もすり抜けた
——平文の本文も封緘された本文も、届くし表示される。違うのはディスク
だけ。本番で動いたまま「未移植」としてここに残っていた。

`delivery::seal_inbound` が Go の順序を踏襲する:

1. 既に PGP の本文はそのままにして `$e2e` を付ける。再暗号化すると、
   受信者が読める暗号文を、リレーの相手の鍵でしか開かない層で包む
   ことになる。
2. そうでなく公開鍵が置いてあれば封緘する。添付は先に
   `multipart/mixed` に畳む——`parse_mime_email` が本文だけに縮めた
   後なので、本文だけ封緘すると添付が黙って消える。
3. 鍵が無ければ平文のまま保存する。配信を断るほうが悪く、鍵を上げる
   ことがこれを有効にする操作。

`inbound_sealing_interop` は**暗号文を比較しない**（セッション鍵が毎回
違うので、比べるのは乱数になる）。Go の OpenPGP に復号させ、平文が
戻ることと、保存されたメッセージのどこにも平文が残っていないことを
見る。受信者のクライアントが向こう側にいるので、それが本番に近い。

### 11.24 起動時に DKIM 鍵と DNS を突き合わせる（Go に無い）

Go は自分が署名に使う鍵が DNS に公開されているかを確認しない。移植側は
起動後にドメインごとに 1 回 TXT を引き、`data/<domain>/key.pem` から
導いた公開鍵と比べて、食い違い・不在を **warn** で報告する。

これは本番で 1 か月以上見えていなかった障害への対処:

- `biset.md` は `data/biset.md/key.pem` で署名し続け、
  `default._domainkey.biset.md` には**別の鍵**が公開されていた。
  対応する秘密鍵は v1 にも v2 にも無い。**全署名が検証に失敗していた。**
- `t.biset.md`（アカウントの発行先、21 個）はレコードそのものが無かった。

大半の受信者は検証に失敗した署名を黙って受け取るので、症状が出ない。
最初に拒否したのは chatmail（Delta Chat）で、`554 5.7.1 No valid DKIM
signature found` が唯一の手がかりだった。**秘密鍵を持ち、公開鍵を引ける
のはリレーだけ**で、コストはドメインごとに 1 回の lookup。

致命的にはしない。署名が不正でも大半には届くので、起動を拒否すると
配信性の問題を停止に変えてしまう。DNS が引けないことは何の証拠でもない
ので `NoAnswer` として区別し、不一致とは呼ばない（所有証明と同じ
fail-open）。

判定は `dkim_dns::decide` に切り出してある。分割されて返る TXT、タグの
順序違い、base64 中の空白——どれも「不一致」と誤報すると、運用者が
警告を無視することを学習してしまう。

**ログ行が増えるため difftest には `dkim-dns-check` フィルタがある。**
Go に対応する行が無いので、内容ではなく行の有無が差になる。

---

### 11.25 一時失敗したメールを保持して再送する（Go に無い）

Go の `smtpSend` の失敗は `sendMessage` からそのまま JMAP クライアントへ
返り、メールは消える。再送も記録も無い。`smtp_out` の doc は
「the message fails and the queue above retries」と書いているが、上に
キューは無かった。

**グレイリスティング**——未知の差出人の初回を 4xx で断り、数分後の再送を
受け入れる——は相当数のサーバの標準的な運用で、この relay が送った
グレイリストされたメールは全部失われ、差出人には「そのアドレスは
使えない」と伝わっていた。

移植側は `data/_queue/<id>/` に保持する:

| 判定 | 例 | 動作 |
|---|---|---|
| 一時 | 4xx、接続失敗、MX が引けない | 保持して再送 |
| 永続 | 5xx、宛先が無い／不正 | そのままエラーを返す |

コードが読めない返信は**一時**扱い。もう一度試す代償は小さく、
読み違えで捨てる代償はメールそのもの。

再送間隔は 1分 / 5分 / 15分 / 1時間 / 2時間 / 4時間 / 8時間 で打ち切り
（計 15.5 時間）。Postfix の既定は 5 日だが、ここはチャット的なメールを
運ぶので、1 日遅れて届くより「届かなかった」と伝わるほうが有用な場面が
多い。

**クライアントへの返答が Go と違う。** 一時失敗は成功として返す。SMTP で
4xx を受けて保持したなら、責任を引き受けたのはリレーだからで、これが
唯一クライアント側から観測できる差異。

activity ログの `result` は `ok` / `queued` / `failed` の 3 値になる。
metric は動かさない——保持中のメールにはまだ結果が無く、送信済みと
数えれば起きていない配送を報告し、失敗と数えれば起きるかもしれない
ものを否定する。結果が判った時点で再送ループが数える。これは
difftest が比較する metric の形を Go と同じに保つことにもなる。

打ち切ったときは差出人自身の受信箱に配送失敗通知を入れる。これが無いと
キューは沈黙を後ろにずらすだけになる。外部の `MAIL FROM` へバウンスは
返せないが、ここでの差出人は必ずこのリレーのアカウントなので、本人が
見ている場所に置ける。

`data/_queue` は起動時のスイープの除外リストに入っている。入れ忘れた
まま実装した時点では、**一時失敗を生き延びたメールが次のデプロイで
消える**状態だった——キューが防ごうとしている損失が別の経路で起きる。
空ディレクトリが消えても何も見えないので、
`the_outbound_queue_is_not_swept` だけがそれを示す。

---

### 11.26 WKD は「config に書かれた」ではなく「実際に配送する」アカウントに答える（Go と差異）

Go の WKD は `cfg.Domains[d].Accounts[localpart]` を見る。移植も同じに
していた。

**本番の config はアカウントを 1 つも宣言していない。** biset は実行時に
アカウントを作るので、`biset.md` も `t.biset.md` も account マップが空。
結果として WKD は**全アドレスに 404 を返し続けていた** — 鍵は各アカウント
のディレクトリに置いてあるのに。両実装とも、稼働していた全期間。

誰も報告しなかったのは、これで困るのが「一度もやり取りしていない biset の
アドレスに GnuPG や Thunderbird で暗号化したい他人」だけで、その人には
「鍵が見つからない」としか見えないため。

移植側は配送に使うのと同じ登録簿（`accounts`）に訊く。鍵が公開されるのは
そのアドレスが実際に配送対象であるときだけ、という条件は変わらない
——`resolve_wkd` に渡す述語が「配送する **かつ** 鍵がある」を 1 つの
問いとして答える。

**欠陥は `resolve_wkd` の中ではなく、ハンドラが何を訊いていたかにあった。**
述語はテスト側が与えるので、関数の単体テスト 21 件は config だけを見る
実装でも全部通る。`wkd_serves_runtime_accounts` はルートを HTTP で叩き、
実行時に作られたアカウントに対して答えることを要求する。変異（config
だけを見る旧実装）で単体テストは 21 件とも緑のまま、こちらだけが赤に
なることを確認済み。
...
**実装済み** (M8)。`Step::divergence` に
**「何が」違うか（`Aspect`）と「なぜ」（SPEC の節番号）**を書く。

宣言された側面**だけ**が「違うこと」を要求され、
**それ以外は通常どおり比較される** —
1 つ目の差異が 2 つ目を隠さないため。

> 最初の実装は「ステップ全体が同一か」で判定していた。
> `/metrics` は**どのみち body が違う**（Go ランタイムのコレクタは移植側に対応物が無い）ので、
> **status の修正が失われても「まだ違う」として通っていた。**
> 側面を名指しする形に直して、この見落としが落ちるようになった。

`--both-oracle` と `--self-test` では**宣言を無効化する**。
同じ実装同士なので、宣言された差異が現れないのは当然であり、
そこで要求すると**唯一の「異常でない理由」で自己検査が落ちる**。
