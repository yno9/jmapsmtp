...
## 11. 意図的に Go 版と変える唯一の点

`debug_dump_eml`（既定 `false`）。
Go 版は受信・送信のたびに `/tmp/jmapsmtp-last-{in,out}.eml` に生メールを書く。
平文が残るため既定を off にする。**差分テスト時は `true` にして Go 版と揃える。**

これ以外に意図的な差異を作らない。作るときは必ずこの節に追加する。
