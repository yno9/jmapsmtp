...
oracle:
    ...
    # `-i.bak` then delete: BSD sed (macOS) requires an argument to -i,
    # and GNU sed (Linux) accepts one, so this is the form that runs on
    # both. The bare `-i` ran only on the machine it was written on.
    sed -i.bak 's|=> /Users/n/go-jmapserver|=> {{go_jmapserver}}|' oracle/go-jmapsmtp/go.mod
    rm -f oracle/go-jmapsmtp/go.mod.bak
...
# Build on s2 and install on v1, in one command.
#
# `just check` runs here; the binary that ships is built on s2. The Mac is
# arm64 Darwin and v1 is x86_64 Linux, and cross-compiling is a toolchain
# project rather than a flag: web-push pulls isahc -> curl (C), and ring wants
# a C compiler too. Building on the same OS and architecture as production is
# also the only way the binary that actually ships is ever built there.
#
# Refuses a dirty tree, and records the commit in /root/jmapsmtp/DEPLOYED_COMMIT
# so "what is running" has an answer.
deploy *ARGS:
