...
### v1（本番）への移行 — 完了 2026-08-10

`mail.biset.md`（ポート 25、23 アカウント / 238 通）を Rust 版に切り替えた。
**2 度ロールバックしてから成功**している。

見つかった実データ消失バグ 2 件（どちらも複製に対する事前検証で判明）:

1. `"account": null` を serde が拒否し、カスタムドメインが未登録に見えて
   **アカウントごと削除**された（`71bfc10`）
2. 再起動後の動的アカウントが**配送先として登録されず**、`250` を返して
   捨てていた（`cc7d75e`）。**設定に静的アカウントが 0 件のリレーでは
   全受信メールが失われる**

2 件目はローカルのテストでは絶対に出なかった ――
**移植側のテストは全部「設定に書かれたアカウント」を使っていた**から。

副次的に、公開状態だった `/admin/*` を閉じた
（誰でも `POST /admin/drain-anchor` で 27 件のクレームを release できた）。
...
1. ~~**`Email/query`（1,000 通）が Go より 10〜20% 遅い**~~
   → **解消**（0.83× → **2.59×**、2.17ms → 0.70ms）。
   原因は `Store::all()` の**ディープ clone**。
   Go の struct コピーは string/slice の**ヘッダ**だけを複製するが、
   Rust の `.cloned()` はヘッダ文字列もマップも全部確保し直す。
   query は id しか要らないので `Store::matching_ids` で
   ロック内側で絞るようにした。常駐メモリも 16.3MB → 11.3MB。

   **順序は `all()` と完全一致でなければならない**（クライアントがページングする）。
   両方とも BTreeMap 順に走査し、**安定ソート**で負のキーを使う。
   これを固定するテストは、**60 件必要だった** —
   5 件では unstable ソートに変異させても同じ順序が出て通ってしまった
2. ~~**本番相当での 24 時間動作**~~ → **108.4 時間（4.5 日）連続稼働で満たした。**

   `jmapsmtp-health.timer` が 5 分ごとに記録した 1,289 サンプル
   （2026-08-06 09:02 → 08-10 21:25、壊れた行 0、10 分超の欠測 0）:

   | | |
   |---|---|
   | 異なる PID | **1**（再起動もクラッシュもゼロ） |
   | `state != active` | 0 |
   | HTTP != 200 | 0 |
   | SMTP 到達不可 | 0 |
   | メッセージ件数 | 全サンプルで **2**（消失なし） |
   | RSS 中央値 | 2.48 MB |
   | RSS 最初の 50 / 最後の 50 の平均 | 4.85 MB → **1.96 MB**（漏れではなく落ち着いた） |

   ログは全期間で**起動 1 行と自分で叩いた drain 2 行のみ**。

   **ただし試験環境であり、実トラフィックは無い。**
   これが示すのは「アイドルで落ちない・漏れない・ずれない」ことで、
   **メール量の下での挙動ではない**。
