...
fn the_signed_statement_embeds_a_webvh_did_verbatim() {
    ...
    assert_eq!(
        session_login_statement(WEBVH, "KEY", "mail.biset.md", "nonce123", 1),
        format!("session:{WEBVH}:KEY:mail.biset.md:nonce123:1")
    );
}
...
fn the_signed_statements_have_their_exact_shape() {
    assert_eq!(
        session_login_statement("did:dht:abc", "KEY", "mail.example.com", "abc123", 1700000000),
        "session:did:dht:abc:KEY:mail.example.com:abc123:1700000000"
    );
    assert_eq!(
