...
/// The domains the fixture serves, sorted — what `resolve_wkd` now takes.
/// Whether an address exists is the caller's question, answered by the
/// closure, so the oracle's config no longer has to be mirrored here.
fn served_domains() -> Vec<String> {
...
fn a_mismatched_localpart_and_hash_yields_nothing_on_either_implementation() {
    ...
    for (name, hash, l) in [
        ...
        assert_eq!(
            resolve_wkd(&served_domains(), &hash, l, false, has_key),
            WkdLookup::NotFound,
...
fn an_account_with_no_key_and_no_global_key_is_not_found() {
    ...
    assert_eq!(
        resolve_wkd(&served_domains(), &wkd_hash("bob"), "bob", false, |_, _| {
            false
        }),
        WkdLookup::NotFound
    );
...
fn an_account_with_no_key_falls_through_to_the_relay_wide_one() {
    ...
    assert_eq!(
        resolve_wkd(&served_domains(), &wkd_hash("bob"), "bob", true, |_, _| {
            false
        }),
        WkdLookup::GlobalKey,
        "this port agrees"
...
fn a_mismatched_localpart_does_not_fall_through_to_the_relay_wide_key() {
    ...
    for (name, hash, l) in [
        ...
        assert_eq!(
            resolve_wkd(&served_domains(), &hash, l, true, |_, _| false),
            WkdLookup::NotFound,
            "{name}: this port disagreed"
        );
    }
...
fn a_capitalised_localpart_gets_the_relays_key_from_the_oracle_and_alices_from_this_port() {
    ...
    assert_eq!(
        resolve_wkd(
            ...
            "Alice",
            true,
            |d, l| { jmapsmtp::wkd::pubkey_file(&data, d, l).exists() }
        ),
        WkdLookup::UserKey {
            domain: "a.test".into(),
            localpart: "alice".into()
