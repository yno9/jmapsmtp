...
{
    handle_inner(stream, cfg, backend, Greeting::Send).await
}

/// Whether this session opens with the `220` banner.
///
/// **A session resumed after `STARTTLS` must not send one.** The banner is the
/// answer to the *connection*, not to a command, and the client has already
/// had it; sending a second one puts an extra reply on the wire and every
/// later reply answers the previous command. A real MTA sees its `EHLO`
/// answered by a `220`, its `MAIL` answered by the `EHLO` lines, and its
/// `DATA` answered by the reply to `RCPT` — which is exactly what a bounce
/// from a Postfix relay reported:
///
/// ```text
/// Protocol error: host mail.biset.md said:
///   250 2.0.0 I'll make sure <y@biset.md> gets this (in reply to DATA command)
/// ```
///
/// go-smtp calls `greet()` once, from `Serve`; its post-upgrade `init()`
/// rebuilds the reader and writer and nothing else.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum Greeting {
    Send,
    /// Already sent before the upgrade.
    AlreadySent,
}

/// As [`handle`], for a session resumed on an upgraded connection.
...
where
    S: AsyncRead + AsyncWrite + Unpin + Send,
{
    handle_inner(stream, cfg, backend, Greeting::AlreadySent).await
}
