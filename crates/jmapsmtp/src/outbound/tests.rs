...
/// With no resolver installed there are no exchangers, so nothing is sent
/// anywhere unintended — it is **queued**.
///
/// This used to assert an error. "No MX" and "DNS did not answer" arrive here
/// as the same empty answer, and treating that as final loses mail every time
/// a resolver hiccups. Fifteen hours of retries and then a report to the
/// sender is the safer reading of an ambiguous silence, and it is what every
/// other MTA does.
#[tokio::test]
async fn a_send_with_no_resolver_is_queued_rather_than_guessing() {
    let state = relay(r#"{"domain":{"a.test":{"account":{"alice":{}}}},"hostname":"mx.a.test"}"#);
    let account = state.accounts.get("alice@a.test").unwrap();
    let outcome = send(&state, &account, &message("hi"), &envelope())
        .await
        .expect("queued, not refused");
    assert_eq!(outcome, crate::outbound::Outcome::Queued);

    // Queued means *held*: the bytes are on disk with the envelope, ready for
    // a retry the next process can make.
    let queued = crate::queue::load_all(&state.data_dir);
    assert_eq!(queued.len(), 1, "nothing was actually held");
    assert_eq!(queued[0].to, ["bob@x.test"]);
    assert!(crate::queue::message(&state.data_dir, &queued[0].id).is_ok());
}
