...
impl RelayState {
    ...
    /// Register one dynamic account: open its store, make it deliverable, and
    /// remember it as dynamic.
    ///
    /// **All three, always.** Recording the address in `dyn_accounts` without
    /// opening a store and adding an alias leaves it listed by `/admin/*` and
    /// invisible to delivery — mail is accepted with a `250` and dropped,
    /// which is what happened to every account on a relay whose config
    /// declares none statically. Go does the three together in
    /// `addDynAccount`, and this exists so the two callers (provisioning, and
    /// the restart scan) cannot drift apart again.
    ///
    /// The log line is Go's, word for word: an operator greps for it after a
    /// restart to see which accounts came back.
    pub fn register_dyn_account(self: &Arc<Self>, localpart: &str, domain: &str) {
        let email = format!("{localpart}@{domain}");
        let dir = crate::auth_env::account_dir(&self.data_dir, domain, localpart);
        let store = match jmapserver::Store::open(&dir) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("[provision] store error for {email}: {e}");
                return;
            }
        };
        ...
        // store cannot mint an id, a Message-ID or a receive time — so without
        // hooks a create answers `serverFail: Email/set create not
        // configured`. Go installs them inside `makeStore`, which every
        // account goes through; here they are a separate step, so any path
        // that opens a store has to remember it. This one did not, and on a
        // relay whose accounts are all dynamic that meant **nobody could
        // send**.
        if let Some(account) = self.accounts.get(&email) {
            crate::submit::install_hooks(self, &account);
        }
        println!("[provision] registered account {email}");
    }
...
/// Credentials for the **JMAP** routes, which accept three forms where the
/// relay's own routes accept only Basic.
///
/// Port of `go-jmapserver/server.go`'s `extractCredentials`, and the order is
/// its order. The relay's own handlers deliberately keep the narrower rule
/// (`auth_env.go`'s `authenticate`, Basic only) — two different surfaces with
/// two different contracts, and conflating them would widen the relay's.
///
/// **biset uses two of these three and never Basic**: `Bearer <email>:<token>`
/// on `/jmap/api/`, and `?access_token=<email>:<token>` on
/// `/jmap/eventsource/`, because `EventSource` cannot set a header. Accepting
/// only Basic — which is what this port did — leaves a real client unable to
/// authenticate at all while every test using Basic passes.
///
/// A token with no colon is `("", token)`, Go's legacy single-password shape.
/// It is carried through rather than rejected here: refusing it at this layer
/// would move the decision away from the authenticator that owns it.
fn extract_credentials(req: &Request<()>, query: &str) -> Option<(String, String)> {
    use base64::Engine as _;

    let split = |tok: &str| -> (String, String) {
        match tok.split_once(':') {
            Some((u, p)) => (u.to_string(), p.to_string()),
            None => (String::new(), tok.to_string()),
        }
    };
...
/// Authenticate a **JMAP** route. See [`extract_credentials`].
pub fn authenticate_jmap(
    state: &RelayState,
    req: &Request<()>,
    query: &str,
) -> Option<(String, String)> {
    let (username, password) = extract_credentials(req, query)?;
    let id = crate::auth_env::authenticate(
        &state.cfg,
        &state.dyn_accounts,
        &state.data_dir,
...
async fn dispatch(
    ...
    // The JMAP routes accept a credential in the query string, so it has to
    // reach them (`extract_credentials`). Taken here rather than per handler
    // because the body is consumed below and `req` is rebuilt without it.
    let query = req.uri().query().unwrap_or("").to_string();
    ...
    match pattern {
        ...
        "/.well-known/jmap" => handlers::jmap_session(&state, &req, &query),
        "/jmap/api/" => handlers::jmap_api(&state, &req, &body, &query),
        ...
        "/jmap/push/subscribe" => handlers::push_subscribe(&state, &req, &body, &query),
        "/jmap/push/unsubscribe" => handlers::push_unsubscribe(&state, &req, &body, &query),
        "/jmap/eventsource/" => handlers::event_source(&state, &req, &query),
...
mod handlers {
    ...
    pub fn wkd_lookup(state: &RelayState, req: &Request<()>) -> Response<Body> {
        ...
        // Every domain this relay serves, sorted: the configured ones and the
        // custom domains verified in an earlier run. Config alone would miss
        // exactly the accounts that exist in practice.
        let mut domains: Vec<String> = state.cfg.domains.keys().cloned().collect();
        domains.extend(state.dynamic_domains.names());
        domains.sort();
        domains.dedup();
        ...
        let key = match crate::wkd::resolve_wkd(
            ...
            // Served **and** holding a key. `accounts` is the registry
            // delivery uses, so a runtime-provisioned account is visible here
            // for the same reason its mail is.
            |d, l| {
                state.accounts.resolve(&format!("{l}@{d}")).is_some()
                    && crate::wkd::pubkey_file(data_dir, d, l).exists()
            },
        ) {
    ...
    pub fn contacts_put(state: &RelayState, req: &Request<()>, body: &[u8]) -> Response<Body> {
        ...
        // **Percent-decoded, because Go's `r.URL.Path` already is.** net/http
        // hands the handler the decoded path, so `TrimPrefix` there yields
        // `urn:uuid:…` where this yields `urn%3Auuid%3A…` untouched. The uid
        // is then compared against the one in the body and the two never
        // match: every contact write from a browser answered 400.
        //
        // Found in a production access log —
        // `PUT /contacts/urn%3Auuid%3A… -> 400` — after DIDComm messages
        // stopped appearing, because biset caches the peers it resolves as
        // contacts.
        let uid = percent_decode(
            req.uri()
                .path()
    ...
    pub fn admin_account_detail(state: &RelayState, req: &Request<()>) -> Response<Body> {
        ...
        // Decoded for the same reason as `/contacts/`: an address carries an
        // `@`, which a browser sends as `%40`.
        let addr = percent_decode(
            req.uri()
                .path()
    ...
    pub fn jmap_session(state: &RelayState, req: &Request<()>, query: &str) -> Response<Body> {
        let mut res = match authenticate_jmap(state, req, query) {
    ...
    pub fn jmap_api(
        ...
        let mut res = (|| {
            if authenticate_jmap(state, req, query).is_none() {
    ...
    pub fn push_subscribe(
        ...
        let mut res = (|| {
            let Some((domain, localpart)) = authenticate_jmap(state, req, query) else {
                return unauthorized_jmap();
            };
            if req.method() != axum::http::Method::POST {
                return method_not_allowed();
    ...
    pub fn event_source(state: &RelayState, req: &Request<()>, query: &str) -> Response<Body> {
        let mut res = (|| {
            if authenticate_jmap(state, req, query).is_none() {
                return unauthorized_jmap();
            }
