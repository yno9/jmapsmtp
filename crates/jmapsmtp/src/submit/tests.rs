...
/// The send happens off the request, so a failure surfaces in the activity log
/// rather than in the response. That is why the log exists.
#[tokio::test]
async fn a_failed_send_is_recorded_and_the_message_is_still_stored() {
    let state = one_account();
    ...
    assert_eq!(
        events[0].result, "failed",
        "there is no MX for that domain and no resolver installed"
    );
    ...
    assert_eq!(state.smtp_outbound(), (0, 1), "counted as a failure");
}
