...
fn a_registered_custom_domain_can_be_provisioned_onto() {
    ...
    assert_eq!(may_provision(&Config::default(), &dom_cfg, "", "s3cret"), Ok(()));
}
...
fn an_open_domain_needs_no_secret() {
    ...
    assert_eq!(may_provision(&Config::default(), &open, "", ""), Ok(()));
}
...
fn a_gated_domain_needs_the_right_secret() {
    ...
    assert_eq!(may_provision(&Config::default(), &gated, "", "s3cret"), Ok(()));
    assert_eq!(
        may_provision(&Config::default(), &gated, "", "wrong"),
        Err(Refusal::DomainNotOpen)
    );
    assert_eq!(may_provision(&Config::default(), &gated, "", ""), Err(Refusal::DomainNotOpen));
}
...
fn a_domain_with_no_secret_and_no_flag_is_not_creatable() {
    ...
    assert_eq!(
        may_provision(&Config::default(), &privileged, "", ""),
        Err(Refusal::DomainNotOpen)
    );
    assert_eq!(
        may_provision(&Config::default(), &privileged, "", "anything"),
        Err(Refusal::DomainNotOpen)
    );
}
...
// ── authorized_did_domain: Lv1 gated by Lv2's own condition ─────────────────
//
// Was `Option<String>` naming exactly one trusted did-domain; is now `bool`
// (2026-08-18, ARC.md §2.9) — when set, Lv1 itself is refused unless the
// identity's did:webvh document already lives on a domain THIS relay
// administers (any member of `config.domains`), the exact membership test
// `did::alias_reconcile::desired_alias` runs continuously for Lv2. There is
// no username to compare any more: Lv1's localpart is always the SCID
// projection, never a submitted name.
...
/// A relay administering two domains, one of which gates Lv1 on the
/// identity's did:webvh document already living somewhere THIS relay
/// administers — which may be a DIFFERENT domain than the one being gated.
fn gated_multi_domain() -> Config {
    cfg(r#"{"domain":{
        "mail.test":{"authorized_did_domain":true},
        "other.test":{"allow_provision":true}
    }}"#)
}

/// The identity's did:webvh document lives at `other.test` — not
/// `mail.test` itself, but still a domain this same relay administers.
#[test]
fn an_identity_whose_webvh_domain_is_administered_here_is_admitted() {
    let cfg = gated_multi_domain();
    let dom_cfg = cfg.domains["mail.test"].clone();
    assert_eq!(
        may_provision(&cfg, &dom_cfg, &webvh("other.test", "alice"), ""),
        Ok(())
    );
}

/// **The relay's own domain is not a special case.** `mail.test` is a member
/// of its own domain set, the same as any other — there is one rule
/// (membership), and "mine" is not an exception to it.
#[test]
fn an_identity_rooted_at_the_gating_domain_itself_is_admitted() {
    let cfg = gated_multi_domain();
    let dom_cfg = cfg.domains["mail.test"].clone();
    assert_eq!(
        may_provision(&cfg, &dom_cfg, &webvh("mail.test", "alice"), ""),
        Ok(())
    );
}

#[test]
fn an_identity_from_an_unadministered_domain_is_refused() {
    let cfg = gated_multi_domain();
    let dom_cfg = cfg.domains["mail.test"].clone();
    assert_eq!(
        may_provision(&cfg, &dom_cfg, &webvh("stranger.example", "alice"), ""),
        Err(Refusal::DidDomainNotAuthorized)
    );
}

/// A domain gating Lv1 by home-domain membership admits **only** identities
/// it can read a domain out of: a `did:dht`, or anything else unreadable,
/// has no domain to check rather than a rejected one.
#[test]
fn a_did_with_no_readable_domain_is_a_malformed_request() {
    let cfg = gated_multi_domain();
    let dom_cfg = cfg.domains["mail.test"].clone();
    assert_eq!(
        may_provision(&cfg, &dom_cfg, "did:dht:abc", ""),
        Err(Refusal::DidNotReadable)
    );
    assert_eq!(may_provision(&cfg, &dom_cfg, "", ""), Err(Refusal::DidNotReadable));
    // The legacy `dids:` path form, which names a different log URL.
    assert_eq!(
        may_provision(
            &cfg,
            &dom_cfg,
            &format!("did:webvh:{SCID}:mail.test:dids:alice"),
            ""
        ),
        Err(Refusal::DidNotReadable)
    );
...
/// **The strictest mode wins.** An operator who opted into gating Lv1 by
/// domain membership said something more specific than "open"; letting
/// `allow_provision` also answer would discard it silently and admit
/// everyone.
#[test]
fn it_overrides_allow_provision_rather_than_adding_to_it() {
    let cfg = cfg(
        r#"{"domain":{"mail.test":{"allow_provision":true,"authorized_did_domain":true}}}"#,
    );
    let dom_cfg = cfg.domains["mail.test"].clone();
    ...
    assert_eq!(
        may_provision(&cfg, &dom_cfg, &webvh("stranger.example", "alice"), ""),
        Err(Refusal::DidDomainNotAuthorized),
        "allow_provision must not rescue an unadministered domain"
    );
}

/// The same precedence against the secret gate: no secret, right or wrong,
/// substitutes for the domain-membership check.
#[test]
fn it_overrides_the_secret_gate_too() {
    let cfg = cfg(
        r#"{"domain":{"mail.test":{"provision_secret":"s3cret","authorized_did_domain":true}}}"#,
    );
    let dom_cfg = cfg.domains["mail.test"].clone();
    assert_eq!(
        may_provision(&cfg, &dom_cfg, &webvh("stranger.example", "alice"), "s3cret"),
        Err(Refusal::DidDomainNotAuthorized)
    );
    // And an administered-domain identity needs no secret at all.
    assert_eq!(
        may_provision(&cfg, &dom_cfg, &webvh("mail.test", "alice"), ""),
        Ok(())
    );
...
/// `false` is the default, and must leave every existing config behaving
/// exactly as it did — the DID is not looked at.
#[test]
fn false_is_the_default_and_changes_nothing() {
    let cfg = cfg(r#"{"domain":{"mail.test":{"allow_provision":true}}}"#);
    let dom_cfg = cfg.domains["mail.test"].clone();
    assert!(!dom_cfg.authorized_did_domain);
    assert_eq!(
        may_provision(&cfg, &dom_cfg, "did:dht:whatever", ""),
        Ok(()),
