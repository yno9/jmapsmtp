    ...
    tokio::spawn(async move {
        ...
        let _ = jmapserver::activity::append_activity(
            ...
            &jmapserver::activity::ActivityEvent {
                ...
                result: match &result {
                    Ok(o) => o.activity_result(),
                    Err(_) => crate::handler::activity_result(false),
                }
                .into(),
                ..Default::default()
            },
        ...
        match result {
            ...
            Ok(crate::outbound::Outcome::Queued) => {
                // Neither counter moves: the message has no outcome yet.
                // Counting it as sent would report a delivery that has not
                // happened, and as failed would report one that may still
                // succeed. The retry loop records whichever it turns out to
                // be. This also keeps the exported metrics the same shape as
                // Go's, which `difftest` compares.
            }
            Ok(crate::outbound::Outcome::Sent(sent_message_id)) => {
