...
// ── ESMTP parameters on MAIL FROM ─────────────────────────────────────────

/// Only what the far end advertised, in `net/smtp`'s order. The wire
/// behaviour is pinned against the Go client in
/// `tests/smtp_client_dialogue_interop.rs`; these fix the edges that a live
/// comparison would need an unusual server to reach.
#[test]
fn mail_from_carries_only_the_advertised_parameters() {
    let bare = "250 server\r\n";
    assert_eq!(mail_from("a@x", bare), "MAIL FROM:<a@x>\r\n");

    let both = "250-8BITMIME\r\n250-SMTPUTF8\r\n250 server\r\n";
    assert_eq!(
        mail_from("a@x", both),
        "MAIL FROM:<a@x> BODY=8BITMIME SMTPUTF8\r\n"
    );

    // Order is `net/smtp`'s, not the server's: 8BITMIME first even when the
    // advertisement came the other way round.
    let reversed = "250-SMTPUTF8\r\n250-8BITMIME\r\n250 server\r\n";
    assert_eq!(mail_from("a@x", reversed), mail_from("a@x", both));

    let one = "250-8BITMIME\r\n250 server\r\n";
    assert_eq!(mail_from("a@x", one), "MAIL FROM:<a@x> BODY=8BITMIME\r\n");
}

/// A keyword is the **first token of a line**, never a substring.
#[test]
fn an_advertisement_is_matched_as_a_whole_keyword() {
    // Carries a parameter, as SIZE does.
    assert!(advertises("250-SIZE 35882577\r\n250 ok\r\n", "SIZE"));
    // The last line has a space where the others have a dash.
    assert!(advertises("250-PIPELINING\r\n250 8BITMIME\r\n", "8BITMIME"));
    assert!(advertises("250-8bitmime\r\n250 ok\r\n", "8BITMIME"));

    // Not a keyword: part of another one, or in the greeting text.
    assert!(!advertises("250-XSMTPUTF8ISH\r\n250 ok\r\n", "SMTPUTF8"));
    assert!(!advertises(
        "250 mail.example.com says 8BITMIME\r\n",
        "8BITMIME"
    ));
    assert!(!advertises("250 server\r\n", "8BITMIME"));
}
