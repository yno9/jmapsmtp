...
///
/// Whether the relay **serves** the address, not whether the address appears
/// in `config.json`. Go asks `cfg.Domains[d].Accounts[localpart]`, and this
/// port copied that: on a relay where accounts are provisioned at runtime —
/// which is what biset does, and what the production deployment is — the
/// config declares no accounts at all, so WKD answered 404 for everyone while
/// their keys sat on disk. Both implementations, the whole time.
///
/// The registry that knows about dynamic accounts is the same one delivery
/// uses; a key that is published has to belong to an address that is served,
/// and that is one question, so the caller answers it in one closure.
/// SPEC.md §11.26.
pub fn resolve_wkd(
    domains: &[String],
    hash: &str,
    localpart_param: &str,
    has_global_key: bool,
    serves_key: impl Fn(&str, &str) -> bool,
) -> WkdLookup {
    let localpart = localpart_param.to_lowercase();
    if !localpart.is_empty() && wkd_hash(&localpart) == hash {
        // `domains` arrives sorted, because Go ranges over the domain map
        // here: with the same localpart on two domains its answer varies
        // between runs, and which key a stranger gets should not
        // (SPEC.md §11.5).
        for domain in domains {
            if serves_key(domain, &localpart) {
