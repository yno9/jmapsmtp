...
fn the_scan_recovers_exactly_what_the_sweep_keeps() {
    ...
    // Collects what the scan would register. The real caller opens a store
    // and adds a delivery alias for each — see
    // `RelayState::register_dyn_account`, and
    // `delivery::tests::a_dynamic_account_receives_mail_after_a_restart` for
    // why recording the name alone is not enough.
    let dynamic = DynAccounts::default();
    scan_dyn_accounts(&config, &dynamic_domains, tmp.path(), |lp, d| {
        dynamic.insert(format!("{lp}@{d}"))
    });
...
fn the_scan_covers_dynamic_domains_too() {
    ...
    scan_dyn_accounts(
        ...
        &dynamic_domains,
        tmp.path(),
        |lp, d| dynamic.insert(format!("{lp}@{d}")),
    );
...
/// `data/_queue/` is not a domain, and the sweep is a `remove_dir_all`.
///
/// Caught by reading the sweep while adding the queue, not by running it: the
/// queue is empty in every other test and an empty directory being deleted
/// looks like nothing at all. It would have shown up in production as mail
/// that survived a temporary failure and then vanished at the next deploy —
/// which is the failure the queue exists to prevent, arriving by a different
/// road.
#[test]
fn the_outbound_queue_is_not_swept() {
    let tmp = tempfile::tempdir().unwrap();
    let entry = crate::queue::enqueue(
        tmp.path(),
        "a@static.test",
        &["b@far.test".to_string()],
        b"From: a@static.test\r\n\r\nheld\r\n",
        "450 greylisted",
    )
    .unwrap();

    let config = cfg(r#"{"domain":{"static.test":{}}}"#);
    let removed = cleanup_orphaned_data(&config, &DynamicDomains::default(), tmp.path());

    assert!(
        !removed.iter().any(|r| r.contains("_queue")),
        "the sweep deleted the outbound queue: {removed:?}"
    );
    assert_eq!(
        crate::queue::load_all(tmp.path()).len(),
        1,
        "the queued message did not survive startup"
    );
    assert!(crate::queue::message(tmp.path(), &entry.id).is_ok());
}
