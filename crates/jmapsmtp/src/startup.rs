...
use crate::auth_env::{DynAccounts, account_dir, read_auth_hash, read_envelope};
...
/// custom-domain registry. Both sit exactly where an account directory would.
const RESERVED_DOMAIN_DIRS: &[&str] = &["_domains"];
...
pub fn scan_dyn_accounts(
    ...
    for domain in domains {
        ...
        for entry in entries.flatten() {
            ...
            if !read_auth_hash(data_dir, &domain, &localpart).is_empty() {
                dynamic.insert(format!("{localpart}@{domain}"));
            }
        }
    }
}
