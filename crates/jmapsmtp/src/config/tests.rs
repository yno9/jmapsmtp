...
// ── nulls a Go relay writes ───────────────────────────────────────────────

/// **The exact bytes a production relay had on disk.**
///
/// `json.Marshal` writes a nil Go map as `null`, and `DomainConfig.Accounts`
/// is nil for every custom domain the Go relay registers — so this is the
/// shape of `data/_domains/<domain>/domain.json` everywhere, not an edge case.
///
/// `#[serde(default)]` does not cover it: default applies to an *absent* key,
/// and this key is present and null. Before `null_as_default` this failed to
/// parse, the domain looked unregistered, and the startup sweep deleted every
/// account on it.
#[test]
fn a_domain_json_written_by_the_go_relay_parses() {
    let real = r#"{"dkim_selector":"default","account":null,"allow_provision":false,"provision_secret":"a06255c541406ba527280ecec0bc430a"}"#;
    let dc: DomainConfig = serde_json::from_str(real).expect("a Go-written domain.json must parse");
    assert!(dc.accounts.is_empty());
    assert_eq!(dc.dkim_selector, "default");
    assert!(!dc.allow_provision);
    assert_eq!(dc.provision_secret, "a06255c541406ba527280ecec0bc430a");
}

/// Every collection Go could marshal from a nil, at every level. A `null` that
/// reaches any of these is a parse failure, and a parse failure on a domain
/// file is data loss.
#[test]
fn null_is_accepted_wherever_go_can_write_it() {
    let cfg: Config =
        serde_json::from_str(r#"{"domain":null,"reply_only_exempt":null,"peer_data_dirs":null}"#)
            .expect("top-level nulls must parse");
    assert!(cfg.domains.is_empty());
    assert!(cfg.reply_only_exempt.is_empty());
    assert!(cfg.peer_data_dirs.is_empty());

    let dc: DomainConfig =
        serde_json::from_str(r#"{"account":{"alice":{"alias":null}}}"#).expect("a null alias list");
    assert!(dc.accounts["alice"].alias.is_empty());
}

/// Null must not become a *different* answer from absent, or a domain with no
/// accounts would behave unlike one whose key was omitted.
#[test]
fn null_and_absent_agree() {
    let from_null: DomainConfig = serde_json::from_str(r#"{"account":null}"#).unwrap();
    let from_absent: DomainConfig = serde_json::from_str("{}").unwrap();
