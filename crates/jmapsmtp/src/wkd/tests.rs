...
/// The domains a relay serves, in the order `resolve_wkd` is given them.
...
/// An address nobody is served at falls through to the relay's own key
/// rather than inventing one.
///
/// The predicate now answers "served **and** holding a key" as one question,
/// so "no such account" is expressed by it saying no — it used to be the
/// config's job, which is what made every runtime-provisioned account
/// invisible.
#[test]
fn an_address_the_relay_does_not_serve_never_resolves_to_a_user_key() {
    assert_eq!(
        resolve_wkd(
            &one_domain(),
            &wkd_hash("nobody"),
...
fn the_same_localpart_on_two_domains_resolves_the_same_way_every_time() {
    // Given to `resolve_wkd` sorted, which is the caller's job — the
    // determinism lives in that ordering, so this passes the pair the way the
    // handler does.
    let cfg = vec!["a.test".to_string(), "z.test".to_string()];
    for _ in 0..20 {
        assert_eq!(
...
/// The production shape: no account is declared in `config.json` at all, and
/// every one of them exists because it was provisioned at runtime.
///
/// This is what was broken. `resolve_wkd` asked the config, the config was
/// empty, and WKD answered 404 for every address on the relay while the keys
/// sat on disk beside their mailboxes — in both implementations, for as long
/// as either had run. Nobody noticed, because the only people who would are
/// strangers using GnuPG or Thunderbird, and they just see "no key found".
#[test]
fn an_account_that_exists_only_at_runtime_is_still_published() {
    // The caller answers "is this address served, and is there a key" — the
    // registry says yes for a dynamic account exactly as it does for a
    // configured one.
    assert_eq!(
        resolve_wkd(
            &["biset.md".to_string()],
...
/// An address the relay does not serve gets nothing, even with a key file
/// left behind by a deleted account.
#[test]
fn a_key_without_a_served_address_is_not_published() {
    assert_eq!(
        resolve_wkd(
            &["biset.md".to_string()],
            &wkd_hash("ghost"),
